...
function termina_intervencao($id, $relatorio, $assinatura, $projeto) {
    ...
    if (intval($id)) {
        ...
        $sql = "select * from bo where ndos = 18 and origem = 'APP' and fref = '" . $projeto . "'";
        ...
        if( !sizeof($documento_aberto) ) {
			$sql = "
                INSERT INTO bo (
                    ...
                    '" . $stamp . "',
                    '" . utf8_decode('Imputação de Custos') . "',
                    '18',
                    '" . $cliente["no"] . "',
                    '" . $cliente["estab"] . "',
                    isnull((select max(obrano) + 1 from bo where ndos = 18 and year(dataobra) = " . date('Y') . "), 1),
                    '" . date('Y') . "',
		...
		$sql = "
			INSERT into bi (
				...
				isnull((select unidade from st where ref = '" . $fixed_ref . "'), ''),
				isnull((select familia from st where ref = '" . $fixed_ref . "'), ''),
				1,
				" . $moh . " * (isnull((select epcusto from st where ref = '" . $fixed_ref . "'), 0)),
				'" . $documento_aberto[0]['bostamp'] . "',
				'" . $assinatura . "',
				'" . strtoupper($fixed_ref) . "',
				isnull((select design from st where ref = '" . $fixed_ref . "'), ''),
				'" . $tipointer . "',
				" . $moh . ",
				" . $moh . " * (isnull((select epcusto from st where ref = '" . $fixed_ref . "'), 0)),
				CONVERT(VARCHAR(10), GETDATE(), 20) + ' 00:00:00.000', 
...
function tabela_projetos($no, $order = 'processo', $fechado = -1) {
    $str_sql = "
        SELECT *
        FROM opc
        WHERE processo != '' ";
    if ($fechado <= 0) {
        $str_sql .= " AND DATAFECHO = '19000101' ";
    }
	else {
		$str_sql .= " AND DATAFECHO <> '19000101' ";
	}
