...
(function () {
  ...
  var osSliderSettings = function (container, params) {
    ...
    var defaults = {
        ...
        isUser:false,
        version : 0,
        setupFonts : ["Open Sans", "Roboto", "Slabo 27px", "Lato", "Roboto Condensed", "More fonts in Pro"]
    };
    ...
    oss.deleteCurrentImage = function (imgId, moduleId){
      ...
      // return;
      jQuerOSS.post("?option=com_ajax&module=os_touchslider&Itemid="+oss.params.ItemId+"&task=delete_image&moduleId="+oss.params.moduleId+"&format=raw",{imgId:imgId},
    ...
    oss.changeStyle = function (){
      ...
      if(jQuerOSS("#object-fit-cover-" + oss.params.moduleId).prop('checked')){
    ...
    oss.resetSlider = function (reinit){
      ...
      params.object_fit_cover = jQuerOSS("#object-fit-cover-" + oss.params.moduleId).prop('checked');
    ...
    oss.resetSliderSettings = function (){
      ...
      jQuerOSS("#height-auto-" + oss.params.moduleId).prop('checked',false);
      jQuerOSS("#object-fit-cover-" + oss.params.moduleId).prop('checked',false);
    ...
    oss.cancelImgEditor = function (){
      ...
      jQuerOSS(container + " .back-image-edit," +
              ...
              container + " .image-time-block span").hide('slow');
    ...
    oss.addNewText= function (el){
      ...
      if(jQuerOSS(container + " .editing-text:visible").length){
        ...
        jQuerOSS(container+" .add-image-text,"+container+" .paste-image-text,"+container+" .back-image-edit,"+container+" .image-time-block").hide();
      ...
      jQuerOSS("#os-slider-"+oss.params.moduleId+" .slide-text.text-active").draggable({
        ...
        drag: function(event, ui){
         if(jQuerOSS(container+" .text-block .text-block-width").val() > 0){
            jQuerOSS("#os-slider-"+oss.params.moduleId+" .text-active").width(jQuerOSS(container+" .text-block .text-block-width").val()+'%');
          }else{
            jQuerOSS("#os-slider-"+oss.params.moduleId+" .text-active").width('auto');
          }
    ...
    oss.cancelTextEditor = function (el){
      ...
      if(jQuerOSS(el).length){
        jQuerOSS(container + " .existing-text,"+container +"  .image-time-block").show();
        jQuerOSS("#os-slider-"+oss.params.moduleId+" .slide-text").each(function(index, el) {
          if(jQuerOSS(this).draggable( "instance" )){
            jQuerOSS(this).draggable("destroy");
          }
    ...
    oss.saveTextEditor = function (el){
      ...
      jQuerOSS(container+" .add-image-text,"+container+" .paste-image-text,"+container+" .back-image-edit,"+container+" .image-time-block").show();
  ...
  oss.saveSliderSettings = function (moduleId,copyimage){
    ...
    jQuerOSS("#os-slider-"+oss.params.moduleId+" .slide-text").removeClass('text-active');
    jQuerOSS.post("?option=com_ajax&module=os_touchslider&Itemid="+oss.params.ItemId+"&task=save_settings&moduleId="+oss.params.moduleId+"&format=raw",
