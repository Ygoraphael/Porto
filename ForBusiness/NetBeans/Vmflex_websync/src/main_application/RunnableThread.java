...
import vmflex_websync.Vmflex_websyncView;
...
public class RunnableThread implements Runnable {
    ...
    {
        String old_message = vmflex_websync.Vmflex_websyncView.jTextArea1.getText();
        vmflex_websync.Vmflex_websyncView.jTextArea1.setText(old_message + message + "\n");
        vmflex_websync.Vmflex_websyncView.jTextArea1.setCaretPosition(vmflex_websync.Vmflex_websyncView.jTextArea1.getDocument().getLength());
    }
    ...
    public static void setOutputReg(int cur, int tot) 
    {
        if(tot >0)
        {
            ...
            vmflex_websync.Vmflex_websyncView.jTextArea1.setText(old_message + "\n" + message );
            vmflex_websync.Vmflex_websyncView.jTextArea1.setCaretPosition(vmflex_websync.Vmflex_websyncView.jTextArea1.getDocument().getLength());
        }
    }
    ...
    public void update_process() throws IOException, ClassNotFoundException, ParseException 
    {
        ...
        try 
        {
            if( config.getSyncElemClientes().equals("1") ) {
            ...
            if( config.getSyncElemFamilias().equals("1") ) {
                setOutput("A Exportar Relacoes Familias-----");
                export_relacoes_familias();
            }
            if( config.getSyncElemArtigos().equals("1") ) {
                setOutput("A Exportar Artigos-----");
                export_artigos();
            }
            if( config.getSyncElemDescontos().equals("1") ) {
                setOutput("A Exportar Descontos Cliente/Artigo-----");
                export_descontos();
            }
    ...
    public void export_descontos() throws IOException, ClassNotFoundException, ParseException
    {
        site = new Site();
        config = new Config();
        ...
        site.setUrl( config.getUrlDescontos() );
        
        Desconto desconto = new Desconto( config.getDbHost(), config.getDbInstance(), config.getDbPort(), config.getDbUser(), config.getDbPass(), config.getDb());
        ArrayList<Desconto> descontos = desconto.list();
        ...
        if ( descontos != null ) 
        {
            if (descontos.size() > 0 ) 
            {
                try 
                {
                    int listSize = descontos.size();
                    setOutput( "A enviar " + listSize + " descontos:");
                    ...
                    {    
                        ...
                        while (pos < listSize) 
                        {
                            ...
                            descontos2 = descontos.subList(pos, pos+page);
                            
                            data = "data=" + 
                            URLEncoder.encode(serializer.serialize( descontos2 ), "UTF-8");
                            ...
                            {
                                WriteToLog( "Erro: sincronizar descontos!" );
                                WriteToLog("Detalhes:");
                                while ((responseText=site.response.readLine()) != null )
                                {
                                    WriteToLog(responseText);
                                ...
                                for( Desconto desc : descontos2) 
                                {
                                    WriteToLog( desc.artigo );
                                }
                            }
                        ...
                        if ( errors == 0 ) 
                        {
                            setOutput( "Descontos actualizados com sucesso." );
                            descontos = null;
                        } 
                        else 
                        {
                            setOutput( "Ocorreram erros na actualização dos descontos. Verificar log." );
                            descontos = null;
                        }
                    } 
                    else 
                    {
                        data = "data=" + URLEncoder.encode(serializer.serialize( descontos ), "UTF-8");
                        ...
                        try 
                        {
                            ...
                            {
                                setOutput( "Ocorreu um erro ao sincronizar descontos! Verificar log." );
                                descontos = null;
                            }
                        ...
                        catch ( Exception e ) 
                        {
                            ...
                            WriteToLog( "Erro: sincronização simples descontos" );
                            WriteToLog("Detalhes:");
                            while( (responseText = site.response.readLine()) != null ) 
                            {
                                WriteToLog( responseText);
                ...
                {
                    WriteToLog("Erro: sincronização geral descontos");
                    WriteToLog("Detalhes");
                    WriteToLog( e.toString() );
                    descontos = null;
                    return;
            ...
            {
                setOutput( "Os descontos já se encontram actualizados" );
                descontos = null;
            }
        }
    }
        ...
        if ( artigos != null ) 
        {
            if (artigos.size() > 0 ) 
            {
                try 
                {
                    ...
                    if (listSize > config.getNumReg()) 
                    {    
                        setOutputReg(0, listSize);
                        int page = config.getNumReg();
                        int pos = 0;
                        int errors = 0;
    ...
    public void export_clientes() throws IOException, ClassNotFoundException, ParseException
    {
        site = new Site();
        config = new Config();
    ...
    public void export_relacoes_familias() throws IOException, ClassNotFoundException, ParseException
    {
        site = new Site();
        config = new Config();
        ...
        site.setUrl( config.getUrlFamilias() + "_relacoes" ); 
        
        Date d = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse( "1900-01-01 00:00:00" );
        
        Familia familia = new Familia( config.getDbHost(), config.getDbInstance(), config.getDbPort(), config.getDbUser(), config.getDbPass(), config.getDb());
        ArrayList<Familia> familias = familia.list_rel( d );
        ...
        if ( familias != null ) 
        {
            if (familias.size() > 0 ) 
            {
                ...
                {
                    int listSize = familias.size();
                    setOutput( "Relacoes Familias para actualizar: " +  listSize);
                    ...
                    {    
                        ...
                        while (pos < listSize) 
                        {
                            ...
                            {
                                WriteToLog( "Erro: sincronizar Relacoes familias!" );
                                WriteToLog("Detalhes:");
                                while ((responseText=site.response.readLine()) != null )
                                {
                                    WriteToLog(responseText);
                        ...
                        if ( errors == 0 ) {
                            setOutput( "Relacoes Familias actualizadas com sucesso." );
                            familias = null;
                        } 
                        else {
                            setOutput( "Ocorreram erros na actualização das Relacoes familias. Verificar log." );
                            familias = null;
                        }
                    ...
                    else 
                    {
                        ...
                        try 
                        {
                            ...
                            {
                                setOutput( "Ocorreu um erro ao sincronizar Relacoes familias! Verificar log." );
                                familias = null;
                            }
                            ...
                            WriteToLog( "Erro: sincronização simples Relacoes familias" );
                            WriteToLog("Detalhes:");
                            while( (responseText = site.response.readLine()) != null ) 
                            {
                                WriteToLog( responseText);
                ...
                {
                    WriteToLog("Erro: sincronização geral Relacoes familias");
                    WriteToLog("Detalhes");
                    WriteToLog( e.toString() );
                    familias = null;
                    return;
            ...
            {
                setOutput( "As Relacoes familias já se encontram actualizados" );
                familias = null;
            }
        } 
        else 
        {
            setOutput( "Ocorreu um erro ao obter os dados das Relacoes familias. Verificar log." );
            familias = null;
        }
    }
