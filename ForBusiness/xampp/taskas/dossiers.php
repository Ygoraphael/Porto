		...
		<div class="container-fluid-full">
			<div class="row-fluid">
				...
				<div id="content" class="span10">
					<?php 
						$current_page = "Dossiers Internos";
						include("breadcrumbs.php");
						
						$campos = get_campos_tabela('BO');
										...
										<select onchange="" id="val1" class="span3 form-control">
											...
											<?php
												foreach($campos as $campo) {
													echo "<option value='" . $campo["nomecampo"] . "'>" . $campo["titulo"] . " - " . $campo["nomecampo"] . "</option>";
												}
											?>
										</select>
	...
	<script>
		function runQuery() {
			...
			jQuery.ajax({
				...
				data: { "action" : "get_docs_cabecalho('bo','nmdos, OBRANO, no, nome, DATAOBRA, ETOTALDEB, bostamp', ' " + window.btoa(tmp_qr) + "');"},
				success: function(data) 
				{
					...
					try {
						...
						$.each(obj, function(index, value) {
							var dados = new Array();
							dados.push(value["nmdos"]);
							dados.push(value["OBRANO"]);
							dados.push(value["no"]);
							dados.push(value["nome"]);
							dados.push(value["DATAOBRA"].substring(0, 10));
		...
		function removeTR(obj) {
			...
			if(tmp_qr.length > 0)
				$('#queryTab').html("select * from bo where \n" + tmp_qr);
			else
				$('#queryTab').html("");
		}
