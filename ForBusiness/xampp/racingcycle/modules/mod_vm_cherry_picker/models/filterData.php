...
//	private $baseQuery = '';
	...
	//	$tables = array("`#__virtuemart_products` as p");
		...
		if ($keyword) {
			...
			$where['base'][] = "vpl.`product_name` LIKE ". $db->quote("%$keyword%");
		}
		...
		if ($lp || $hp) {
			...
			if ($conf->get('include_individual_price_adjustment') != 0) {
				...
				if ($lp && !$hp) {
					$where['prices'][] = "prices.`product_price` >= $lp";
				} else if (!$lp && $hp) {
					$where['prices'][] = "prices.`product_price` <= $hp";
				} else if ($lp == $hp) {
					$where['prices'][] = "prices.`product_price` = $lp";
				} else {
					$where['prices'][] = "prices.`product_price` BETWEEN $lp AND $hp";
				}
	...
	*/
	...
	public function getMinMaxPrices($includeAppliedFilters = false) {
		...
		if ($conf->get('include_individual_price_adjustment') != 0) {
			...
			$columns = "MIN(prices.`product_price`) as min, MAX(prices.`product_price`) as max";
		}
