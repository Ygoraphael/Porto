...
class JUFormControllerPlugins extends JControllerAdmin
{
	
	protected $text_prefix = 'COM_JUFORM_PLUGINS';

	
	public function getModel($name = 'plugin', $prefix = 'JUFormModel', $config = array('ignore_request' => true))
	{
	...
	public function getChildTemplateIdByTree($pk)
	{
		...
		$query = $db->getQuery(true);
		$query->select('n.id')
			->from('#__juform_templates AS n,#__juform_templates AS p')
	...
	public function orderTemplateTreeBeforeDelete($templateString)
	{
		...
		$query->select('tpl.*');
		$query->from('#__juform_templates AS tpl');
		$query->join('', '#__juform_plugins AS plg ON plg.id = tpl.plugin_id');
	...
	public function checkDeleteTemplateHasChild()
	{
		...
		if (!empty($dataTemplateId) && !empty($dataTemplateTree))
		{
			if (implode(',', $dataTemplateId) != implode(',', $dataTemplateTree))
			{
				...
				$html = '<div class="alert alert-warning">' . JText::_('COM_JUFORM_DELETE_TEMPLATE_WARNING_MESSAGE') . '</div>';
				$html .= '<table class="table table-condensed">';
				$html .= '<thead>';
				$html .= '<tr>';
				$html .= '<th>' . JText::_('COM_JUFORM_TEMPLATE_ID') . '</th>';
				$html .= '<th>' . JText::_('COM_JUFORM_TEMPLATE_TITLE') . '</th>';
				$html .= '</tr>';
				$html .= '</thead>';
				$html .= '<tbody>';
	...
	public function remove()
	{
		...
		$query->from('#__juform_plugins AS plg');
		$query->join('', '#__juform_templates AS tpl ON tpl.plugin_id = plg.id');
		$query->join('', '#__juform_template_styles AS style ON style.template_id = tpl.id');
		...
		if ($templateHomeId)
		{
			JError::raiseWarning(500, JText::sprintf('COM_JUFORM_CAN_NOT_DELETE_PLUGIN_X_BECAUSE_IT_IS_EXTENDED_BY_DEFAULT_TEMPLATE_STYLE', $templateHomeId->title));
		...
		$this->setRedirect(JRoute::_('index.php?option=com_juform&view=plugins', false));
	}
}
