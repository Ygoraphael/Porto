 ...
 *				v12 added pagetype manufacturer.products, which is the list page of a manufacturer's products, as opposed to the page
 *					about the manufacturer.
 * 					added productdetails.recommend, productdetails.notify
 *					
 *					
 */
...
class JomGeniusClassVirtuemart extends JomGeniusParent {
	...
	function __construct() {
		...
		$this->product_id	= $jinput->get('virtuemart_product_id');
		$this->view			= $jinput->get('view', '', 'WORD');
		$this->option		= $jinput->get('option');
		$this->category_id	= $jinput->get('virtuemart_category_id');
		$this->layout		= $jinput->get('layout');
		$this->addrtype		= $jinput->get('addrtype');
	...
	function previousPurchaseInfo( $type ) {
		...
		if ( $type == 'previouspurchaseproductids' ) {
			...
			$db		= JFactory::getDBO();
			$db->setQuery( $query );
			if ( version_compare( JVERSION, '3.0', '<' ) ) { // J2.5
	...
	//			$type = 'child_options';		break; // NOT IN J1.7 - not sure if it's here anywhere.
		...
		$man_id = JFactory::getApplication()->input->get('virtuemart_manufacturer_id', 0, 'INT');
...
//			'orders',
		...
		if ($this->view == 'pluginresponse' and JFactory::getApplication()->input->get('task') == 'pluginresponsereceived' ) {
			return 'plugin.response';
		}
		
		if ($this->view == 'orders') {
			if (JFactory::getApplication()->input->get('order_number', null) != null) return 'order.view';
			return 'orders';
		}
		...
		if ($this->view == 'productdetails') {
			...
			if ($this->task == 'recommend') return 'productdetails.recommend'; // normally appears on menuless popup window
			if ($this->layout == 'notify') return 'productdetails.notify';
		...
		if ($this->view == 'category') {
			...
			if (isset($_GET['virtuemart_manufacturer_id'])) return 'manufacturer.products';
	...
	function flypage() {
		...
		return JFactory::getApplication()->input->get( 'flypage' );
	}
