...
class Admin_model extends CI_Model {
    ...
    public function delete_data($data) {
        ...
        foreach ($data['id'] as $value) {
            $this->db->where($data['variable'], $value);
            $result = $this->db->delete($data['table']);
        }
    ...
    public function get_plugin_pages($id) {
        $this->db->select("*");
        $this->db->from('plugin_page');
    ...
    public function create_menu_category($title, $url, $name, $logo, $position, $toggle, $type) {
    ...
    public function create_metatag($name, $content) {
        $data = array(
            'name' => $name,
            'content' => $content
        );

        $this->db->insert('seo_meta_tags', $data);
        $insert_id = $this->db->insert_id();
    ...
    public function update_metatag($id, $name, $content) {
        ...
        $this->db->where('id', $id);
        $sql = $this->db->update('seo_meta_tags', $data);

        return $id;
    }

    public function update_menu_category($id, $title, $url, $name, $logo, $position, $toggle, $type) {
        $toggle2 = $toggle === '0' ? 0 : 1;
        $data = array(
            'title' => $title,
            'url' => $url,
    ...
    public function getMeta_tag_id($id) {
        $this->db->select("*");
        $this->db->from('seo_meta_tags');
        $this->db->where('id', $id);
        return $this->db->get()->row();
    ...
    public function create_plugin($url, $title, $position, $input) {
        $data = array(
            'title' => $title,
            'position' => $position,
            'url' => $url
        ...
        if (sizeof($input)) {
            ...
            $this->db->insert_batch('plugin_page', $data);
        }

        return $insert_id;
    }

    public function update_plugin($id, $url, $title, $position, $input) {
        $data = array(
            'title' => $title,
            'position' => $position,
            'url' => $url
