<?php
/*
 * @version $Id: cron.php,v 1.8 2013/12/10 07:22:17 Jot Exp $
 * @package JotCache
 * @category Joomla 2.5
...
try {
  ...
  $db->setQuery($query);
  $fnames = array_flip($db->loadResultArray());
  $deleted = array();
  $cnt = 0;
    ...
    while (false !== ($file = readdir($handle))) {
      if ($file != "." && $file != "..") {
        ...
        if ($ext == '.php_expire') {
          ...
          if ($time < microtime(true) || !array_key_exists($fname, $fnames)) {
            @chmod($file1, 0777);
            @unlink($file1);
          ...
          if (array_key_exists($fname, $fnames))
            unset($fnames[$fname]);
        }
      }
    }
  ...
  $deleted = array_flip(array_merge($deleted, $fnames));
  $cnt = count($deleted);
  if ($cnt > 0) {
  ...
  exit("JotCache clean cron job finished. $cnt expired cache items deleted.");
} catch (Exception $e) {
