...
JHtml::addIncludePath(JPATH_SITE . '/administrator/components/com_juform/helpers/html');

class JUFormFieldDateTime extends JUFormFieldBase
{
	public function __construct($field = null, $submission = null)
	{
		...
		$this->params->set('regex', 'custom_regex');
	}
	...
	public function getOutput($options = array())
	{
		...
		if ($format != '')
		{
			$value = JHtml::date($this->value, $format);
		}
	...
	public function loadDefaultAssets($loadJS = true, $loadCSS = true)
	{
		...
		if ($this->folder && !isset($loaded[$this->folder]))
		{
			...
			$showDatePicker       = $this->params->get('show_datepicker', 'date');
			$showDatePickerConfig = array();
			if ($showDatePicker == 'date')
			{
				$showDatePickerConfig[] = 'datepicker: true';
				$showDatePickerConfig[] = 'timepicker: false';
			}
			elseif ($showDatePicker == 'time')
			{
				$showDatePickerConfig[] = 'datepicker: false';
				$showDatePickerConfig[] = 'timepicker: true';
			}
			else
			{
				$showDatePickerConfig[] = 'datepicker: true';
	...
	public function getPredefinedValuesHtml()
	{
		...
		$html .= '<span class="add-on btn icon-calendar fa fa-calendar" id="jform_predefined_values_show"></span>';
		$html .= '</div>';
	...
	public function onSearch(&$query, &$where, $search, $forceModifyQuery = false)
	{
		if (is_array($search) && !empty($search))
		{
			...
			{
				$query->join('LEFT', '#__juform_fields_values AS field_values_' . $this->id . ' ON (form.id = field_values_' . $this->id . '.form_id AND field_values_' . $this->id . '.field_id = ' . $this->id . ')');
	...
	protected function loadDatetimePicker()
	{
		...
		{
			$document = JFactory::getDocument();
			$document->addScript(JUri::root(true) . '/components/com_juform/assets/js/jquery.datetimepicker.js');
			$document->addStyleSheet(JUri::root(true) . '/components/com_juform/assets/css/jquery.datetimepicker.css');
			$loaded = true;
		}
	}
