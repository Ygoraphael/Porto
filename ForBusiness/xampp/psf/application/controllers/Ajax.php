...
class Ajax extends CI_Controller {
	...
	function email() {
        $boParams = array("bostamp" => $this->uri->segment(3));
        $bo = $this->dossier->getDossierCab($boParams);

        $data = array();
        if (count($bo)) {
            $data["bo"] = $bo[0];
            $this->template->content->view('ajax/email', $data);
            $this->template->publish();
        }
    }
	...
	function update_encomenda() {
        ...
        $BoParams = array();
        $BoParams["volume"] = $volume;
        $BoParams["peso"] = $peso;
        $BoParams["bostamp"] = $bostamp;
        $response = $this->dossier->update_encomenda($BoParams);
