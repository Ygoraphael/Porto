...
class CustomfiltersModelCustomfilters extends JModelList{
	...
	protected function getListQuery($use_filters=true)
	{
		...
		$query->select('cft.type AS disp_type');
		...
		$query->join('LEFT','#__cf_filtertypes AS cft ON cf.type_id=cft.id');
		...
		if($use_filters){
			...
			if (is_numeric($disp_type)) {
				$where[] = 'cf.type_id = ' . (int) $disp_type;
			} else if ($disp_type === '') {
	...
	public function getAllDisplayTypes(){
		$db=JFactory::getDbo();
		$q="SELECT id, type  FROM #__cf_filtertypes ORDER BY id ASC";
		$db->setQuery($q);
