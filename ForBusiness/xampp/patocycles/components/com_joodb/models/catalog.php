...
class JoodbModelCatalog extends JModelLegacy
{
	...
	public function __construct()
	{
		...
		if (!$this->_joobase->load( $joodbId)) JError::raiseError( 500, JText::sprintf( 'Database is unpublished or not availiable'));
        ...
        $show_data = ($app->input->get('search')==null && $params->get('form_only')==1) ? false : true;
		...
		if (JRequest::getCmd('reset')=="true") {
            ...
            $app->input->set("gs",null);
            $app->setUserState($option.'.sparams',array());
            $app->input->set("sparams",null);
		}		
