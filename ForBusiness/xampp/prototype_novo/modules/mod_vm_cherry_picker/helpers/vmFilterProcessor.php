...
class VMFilterProcessorPT {
	...
	public static function getPriceSelection() {
		...
		if ($include_individual_price_adjustment != 0) {
			...
			if ($lp && $hp && $lp != $hp) {
				$where[] = "(CASE WHEN `calc_value_mathop` = '+' THEN ".
						" pp.`product_price` BETWEEN (". $lp ." - `calc_value`)".
						" AND (". $hp ." - `calc_value`)".
					" WHEN `calc_value_mathop` = '-' THEN ".
						" pp.`product_price` BETWEEN (". $lp ." + `calc_value`)".
						" AND (". $hp ." + `calc_value`)".
					" WHEN `calc_value_mathop` = '+%' THEN".
						" pp.`product_price` BETWEEN (". $lp ." / (1 + `calc_value` / 100))".
						" AND (". $hp ." / (1 + `calc_value` / 100))".
					" WHEN `calc_value_mathop` = '-%' THEN".
						" pp.`product_price` BETWEEN (". $lp ." / (1 - `calc_value` / 100))".
						" AND (". $hp ." / (1 - `calc_value` / 100))".
					" ELSE pp.`product_price` BETWEEN ". $lp ." AND ". $hp .
					" END)";
				...
				$where[] = "(CASE WHEN `calc_value_mathop` = '+' THEN ".
						" pp.`product_price` ". $operation ." (". $search_price ." - `calc_value`)".
					" WHEN `calc_value_mathop` = '-' THEN ".
						" pp.`product_price` ". $operation ." (". $search_price ." + `calc_value`)".
					" WHEN `calc_value_mathop` = '+%' THEN ".
						" pp.`product_price` ". $operation ." (". $search_price ." / (1 + `calc_value` / 100))".
					" WHEN `calc_value_mathop` = '-%' THEN ".
						" pp.`product_price` ". $operation ." (". $search_price ." / (1 - `calc_value` / 100))".
					" ELSE pp.`product_price` ". $operation ." ". $search_price .
					" END)";
			}

		} else {		// do a regular search without discounts
				if ($lp && !$hp) {
					$where[] = "pp.`product_price`>=". $lp;
				} else if (!$lp && $hp) {
					$where[] = "pp.`product_price`<=". $hp;
				} else if ($lp == $hp) {
					$where[] = "pp.`product_price`=". $lp;
				} else {
					$where[] = "pp.`product_price` BETWEEN $lp AND $hp";
				}
			}
		...
		if ($user->id != 0) {
			...
			if ($shoppergroup_id) {
				$where[] = "pp.`virtuemart_shoppergroup_id`=". $shoppergroup_id;
			}
		}
		...
		// if ($lp && !$hp) {
		// 	return "`product_price`>=". $lp;
		// }
		// else if (!$lp && $hp) {
		// 	return "`product_price`<=". $hp;
		// }
		// else if ($lp && $hp && ($hp > $lp)) {
		// 	return "`product_price` BETWEEN ". $lp ." AND ". $hp;
		// }
		if ( !$where)
			return false;
		else {
