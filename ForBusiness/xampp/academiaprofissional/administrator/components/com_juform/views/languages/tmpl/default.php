...
<form
	action="<?php echo JRoute::_('index.php?option=com_juform&view=languages'); ?>"
	...
	<div id="j-main-container" class="span12">
		...
		{
			?>
			<div class="alert alert-danger"><?php echo JText::_('COM_JUFORM_LANGUAGE_FILE_DOES_NOT_EXIST_SAVE_TO_CREATE_NEW_FILE'); ?></div>
		<?php
		} ?>
		...
		<?php if ($this->groupCanDoManage)
		{
			...
			<div class="filter-search input-append pull-left">
				...
				<input type="text" name="filter_search" id="filter_search" class="input-medium"
					placeholder="<?php echo JText::_('COM_JUFORM_FILTER_SEARCH'); ?>"
		...
		<div class="filter-select pull-right">
			...
			if (count($this->fileArr) > 0)
			{
				?>
				<div class="pull-right">
					<select name="item" id="files" class="input-large" onchange="this.form.submit()">
		...
		<table class="table table-striped adminlist">
			<thead>
			<tr>
				<td><?php echo JText::_('COM_JUFORM_LANGUAGE_FILE_COMMENT'); ?></td>
				<td colspan="2"><textarea name="comment" cols="60" rows="6"><?php echo $this->comment ?></textarea></td>
				<td><a class="modal" href="<?php echo $this->share ?>"
						title="<?php echo JText::_('COM_JUFORM_SHARE_FILE'); ?>"
						rel="{handler: 'iframe', size: {x:800, y:450}}">
						<button class="btn btn-info">
							<i class="icon-mail"></i> <?php echo JText::_('COM_JUFORM_SHARE_FILE'); ?></button>
					</a>
				</td>
			</tr>
			<tr>
				<th style="width:5%; text-align: left;"><?php echo JText::_('COM_JUFORM_FIELD_ID'); ?></th>
				<th style="width:25%; text-align: left;"><?php echo JText::_('COM_JUFORM_FIELD_KEY'); ?></th>
				<th style="width:30%; text-align: left;"><?php echo JText::_('COM_JUFORM_FIELD_ORIGINAL'); ?></th>
				<th style="width:40%; text-align: left;"><?php echo JText::_('COM_JUFORM_FIELD_TRANSLATION'); ?></th>
			</tr>
			</thead>
			...
			foreach ($this->original AS $key => $value)
			{
				...
				<tr class="row<?php echo $i % 2 ?>">
					...
					<td>
						...
						<input type="text" name="<?php echo $key; ?>" class="input-xlarge" size="65" value="<?php echo htmlspecialchars($translatedValue); ?>" />
							<a href="#" class="btn btn-mini btn-success save-language">
								<i class="icon-save"></i> <?php echo JText::_('COM_JUFORM_SAVE'); ?>
							</a>
						<?php if ($this->lang == 'en-GB'): ?>
							<a href="#" class="btn btn-mini btn-danger remove-language">
								<i class="icon-remove"></i> <?php echo JText::_('COM_JUFORM_REMOVE'); ?>
							</a>
						<?php endif ?>
						...
						<?php if ($missing)
						{
							?>
							<br />
							<span style="color:red;">*(<?php echo JText::_('COM_JUFORM_LANGUAGE_STRING_DOES_NOT_EXIST'); ?>)</span>
						<?php } ?>
					</td>
				</tr>
				...
				<tr>
					<td colspan="4">
						<a href="#" id="add-language" class="btn btn-mini btn-primary">
							<i class="icon-new"></i> <?php echo JText::_('COM_JUFORM_ADD_LANGUAGE'); ?>
						</a>
					</td>
				</tr>
			<?php endif ?>
					...
					<div class="pagination">
						<div class="limit">
							<?php echo JHtml::_('select.genericlist', $this->limitArr, 'limit',
								'class="inputbox input-mini" onchange="this.form.submit()"', 'value', 'text', $this->state->get('list.limit'))?>
						</div>
