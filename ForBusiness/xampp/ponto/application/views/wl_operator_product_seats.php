...
<div class="col-lg-12 noleftrightmargin" id="seat_buy">
	<div class="col-lg-12 calendar-top-container noleftrightmargin">
		<div class="col-lg-12 calendar-top-title calendar-title3 noleftrightmargin">
			<span class="calendar-top-title-pos">3</span><b>Select seats</b>
		</div>
		<div class="col-lg-12 calendar-top-title calendar-title1 noleftrightmargin">
			<span class="calendar-top-title-pos">1</span><b>Select date</b>
		</div>
		<div class="col-lg-12 calendar-top-title calendar-title2 noleftrightmargin">
			<span class="calendar-top-title-pos">2</span><b>Select time</b>
		</div>
		<div class="col-lg-12 calendar_div">
			<div id="calendar_place"></div>
		</div>
		<div class="col-lg-12 calendar-price noleftrightmargin" style="margin-top:15px;">
		...
		<div class="col-lg-12" style="margin:15px 0px;">
			...
			<a onclick="calendar_page(3);" style="display:none;" class="btn btn-primary calendar-but-2">Continue</a>
		</div>
		<div class="col-lg-12 calendar-but-3" style="margin:15px 0px;">
			<a onclick="calendar_page(2);" class="btn btn-primary">Back</a>
			<a onclick="checkout();" class="btn btn-primary">BUY NOW</a>
		</div>
		<script>
					...
					success : function(data){
						try
						{
							...
							if(parse.length>0)
							{
								...
								{
									var preco = parse[p];
									jQuery(".seat-value[seat="+seat+"]").val(parseFloat(preco["epv1"]).toFixed(2));
								}
							}
							update_prices_seats();
							$(".calendar-top-container").LoadingOverlay("hide");
			...
			function update_prices_seats() {
				...
				$( ".seat-sel" ).each(function() {
					...
					if( jQuery(this).val().trim().length > 0 ) {
						...
						html+= '<div class="price-category clearfix"><span class="price-cat pull-left">' + place + '</span><span class="price-qtt pull-left">1 x €' + value + '</span>';
						html+= '<span class="price-total pull-right">€' + parseFloat(value).toFixed(2) + '</span></div>';
				...
				$.ajax({
					...
					success : function(data){
						try
						{
							...
							var html = '<select onchange="jQuery(\'.calendar-but-2\').css(\'display\', \'inline\');" class="calendar-time-picker clearfix form-control"><option></option>';
							if(parse.length>0)
							{
								for(p in parse)
								{
