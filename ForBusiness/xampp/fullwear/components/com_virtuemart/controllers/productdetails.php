...
class VirtueMartControllerProductdetails extends JController {
	...
	/**
	 * Json task for recalculation of stock
	 *
	...
	public function recalculate_stock () {
		...
		$customPlugin = array();
		$customVariants = JRequest::getVar ('customPlugin', array());
		
		$value = reset($customVariants);
		$value = reset($value);
		$virtuemart_product_id = $value["child_id"];
		
		$product_model = VmModel::getModel ('product');
		
		$stockFormated = array();
		$stock = $product_model->getStock ( $virtuemart_product_id );
		$stockFormated["stock"] = $stock;
		
		// Get the document object.
		$document = JFactory::getDocument ();
		// stAn: setName works in JDocumentHTML and not JDocumentRAW
		if (method_exists($document, 'setName')){
			$document->setName ('recalculate_stock');
		}
		...
		JResponse::setHeader ('Content-Disposition', 'attachment;filename="recalculate_stock.json"', TRUE);
		JResponse::sendHeaders ();
		echo json_encode ($stockFormated);
		jexit ();
	}
