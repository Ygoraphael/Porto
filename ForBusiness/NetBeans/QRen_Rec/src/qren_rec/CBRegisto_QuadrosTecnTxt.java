package fme;

import java.util.Vector;
import javax.swing.JLabel;
import javax.swing.JTextArea;















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































class CBRegisto_QuadrosTecnTxt
  extends CBRegisto
{
  Frame_QuadrosTecn_2 P;
  
  public String getPagina()
  {
    return "QuadrosTecn";
  }
  

  int limite = 3000;
  
  CBRegisto_QuadrosTecnTxt() {
    tag = "TxtQuadrosTecn";
    P = ((Frame_QuadrosTecn_2)fmeApp.Paginas.getPage("QuadrosTecn_2"));
    if (P == null) return;
    started = true;
    
    Campos.add(new CHCampo_TextArea("texto", P.getJTextArea_Fundam(), this));
  }
  
  void on_update(String tag)
  {
    CHCampo chc = getByName(tag);
    if ((chc instanceof CHCampo_TextArea)) {
      JTextArea jta = jcomp;
      int n = jta.getText().length();
      if (tag.equals("texto")) {
        P.jLabel_Count.setText(limite - n + "/" + limite);
      }
    }
  }
  

  CHValid_Grp validar(CHValid_Grp err_list)
  {
    if (err_list == null) {
      err_list = new CHValid_Grp(this, "Fundamentação");
    }
    extract();
    String erro = "Texto demasiado extenso. Por favor, abrevie até " + limite + " caracteres.";
    
    if ((getByName("texto").isEmpty()) && (QuadrosTecnstarted) && (!CBData.QuadrosTecn.isEmpty()))
      err_list.add_msg(new CHValid_Msg("texto", "%o"));
    if ((!getByName("texto").isEmpty()) && (getByName"texto"v.length() > limite)) {
      err_list.add_msg(new CHValid_Msg("texto", erro));
    }
    return err_list;
  }
}
