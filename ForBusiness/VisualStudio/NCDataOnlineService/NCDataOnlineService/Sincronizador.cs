...
namespace NCDataOnlineService
{
    public class Sincronizador
    {
        ...
        private void DoQuery(string name)
        {
            ...
            if (this.config.conexao[text]["tipo_basedados"] == "SQLite")
            {
                string connectionString = SQL.MakeSQLiteConnectionString(this.config.conexao[text]["bd"]);
                using (SQLiteConnection sqlConnection = new SQLiteConnection(connectionString))
                {
                    sqlConnection.Open();
                    SQLiteCommand sqlCommand = new SQLiteCommand(this.config.query[name]["sql"], sqlConnection);
                    int NumRows = 0;

                    using (SQLiteDataReader sqlDataReaderRows = sqlCommand.ExecuteReader())
                    {    
                        while (sqlDataReaderRows.Read())
                        {
                            NumRows = NumRows + 1;
                        }
                    }
            ...
            else if (this.config.conexao[text]["tipo_basedados"] == "MSSQL")
            {
