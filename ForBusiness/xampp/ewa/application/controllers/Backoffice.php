...
class Backoffice extends CI_Controller {
    ...
    public function rep_pickups() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (!$acesso) {
                ...
                $stamp = $this->user_model->get_stamp();
                $data['fields'] = $this->product_model->get_configtable_mssql($stamp, 'tab-pp');
                $data['title'] = 'Pickups';
                $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                $data['pickups'] = $this->product_model->get_pickups_mssql($_SESSION["backoffice_user_id"]);
                $data['pickups_zones'] = $this->product_model->get_pickups_zones_mssql($_SESSION["backoffice_user_id"]);
                $this->template->content->view('backoffice-rep-pickups', $data);
            }
        } else {
            //login or register
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if ($this->input->post('id')) {
                ...
                if (!$acesso) {
                    ...
                    $data['title'] = 'Order num. ' . $data['order'][0]["obrano"];
                    $data['order'] = $data['order'][0];
                    $data['order_bi'] = $this->product_model->get_order_bi_bystamp($this->input->post('id'));
                    $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                    $data['hour'] = $this->product_model->get_hour_mudar_session($data['order']['origem']);
    ...
    public function partialorder() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (trim($order_number) != "") {
                $order_data = $this->phc_model->get_order_num_op_mssql($order_number, $_SESSION["backoffice_user_id"], 1);

                if (sizeof($order_data) > 0) {
                    $data['order_data'] = $order_data[0];
                    $data['title'] = 'Order ' . $data['order_data']['obrano'];
                    $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);

                    if ($data['user']["u_operador"] == 'Sim') {

                        $this->template->content->view('backoffice-partial-order', $data);
                        $this->template->publish();
                    }
                }
            }
    ...
    public function rep_treasury() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (!$acesso) {
                ...
                //$data['fields'] = $this->product_model->get_configtable_mssql($this->user_model->get_stamp() , 'tab-p');
                $stamp = $this->user_model->get_stamp();
                $data['fields'] = $this->product_model->get_configtable_mssql($stamp, 'tab-p');
                $data['title'] = 'Treasury';
                $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                $data['products'] = $this->product_model->operator_get_products_mssql($_SESSION["backoffice_user_id"]);
                $data['agents'] = $this->product_model->operator_get_agents_mssql($_SESSION["backoffice_user_id"]);
    ...
    public function update_config_table() {
        $this->load->model('product_model');
        $fields_dec = (array) json_decode($_POST['field']);
        $form_dec = (array) json_decode($_POST['form']);
        var_dump($form_dec);
        $this->product_model->delete_config_filter($this->user_model->get_stamp(), $fields_dec['table']);
        foreach ($form_dec as $key => $f) {
            $fields[] = $key;
        }
        if (isset($_SESSION['fields'])) {
            foreach ($_SESSION['fields'] as $key => $col) {
                if (!in_array($key, $fields)) {
                    $this->product_model->create_config_filter($this->user_model->get_stamp(), $fields_dec['table'], $key);
                }
            }
        }
    ...
    public function check_fields() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (!$acesso) {
                ...
                $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                $this->load->view('backoffice-checkfields', $data);
            }
        } else {
//login or register
            $data['returnTo'] = 'backoffice';
    ...
    public function rep_reimbursement() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (!$acesso) {
                ...
                $stamp = $this->user_model->get_stamp();
                $data['fields'] = $this->product_model->get_configtable_mssql($stamp, 'tab-pr');
    ...
    public function rep_fees() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (!$acesso) {
                ...
                $stamp = $this->user_model->get_stamp();
                $data['fields'] = $this->product_model->get_configtable_mssql($stamp, 'tab-pf');
                $data['title'] = 'My Fees';
                $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                $data['products'] = $this->product_model->operator_get_products_mssql($_SESSION["backoffice_user_id"]);
                $data['agents'] = $this->product_model->operator_get_agents_mssql($_SESSION["backoffice_user_id"]);
    ...
    public function rep_orders() {
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            ...
            if (!$acesso) {
                ...
                $stamp = $this->user_model->get_stamp();
                $data['fields'] = $this->product_model->get_configtable_mssql($stamp, 'tab-po');
        ...
        if (isset($_SESSION["backoffice_user_id"]) && $_SESSION["backoffice_user_id"] > 0) {
            if ($this->uri->segment(3) != "" && !$this->input->post('id')) {
                ...
                if (!$acesso) {
                    ...
                    $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                    $data['agent'] = $this->phc_model->get_agent($this->uri->segment(3), $_SESSION["backoffice_user_id"]);
                    $data['locations'] = $this->product_model->get_locations_mssql($_SESSION["backoffice_user_id"]);
            ...
            if ($this->input->post('id') && $this->uri->segment(3) != "") {
                ...
                if ($this->input->post('id') == 1) {
                    ...
                    $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                    $data['productfee'] = $this->phc_model->get_productfee_agent($this->uri->segment(3), $_SESSION["backoffice_user_id"], 1);
                    $this->template->content->view('backoffice-agent-product-fee', $data);
                }
                if ($this->input->post('id') == 2) {
                    $data['title'] = 'Assign Products';
                    $data['user'] = $this->user_model->get_backoffice_user($_SESSION["backoffice_user_id"], $_SESSION["backoffice_user_estab"]);
                    $data['productfee'] = $this->phc_model->get_productfee($this->uri->segment(3), $_SESSION["backoffice_user_id"], 1);
        ...
        if (isset($_SESSION["backoffice_user_id"])) {
            ...
            switch ($sefurl) {
                ...
                case 'get_rep_pickups':
                    ...
                    $data["hour_i"] = $this->input->post('hour_i');
                    $data["hour_f"] = $this->input->post('hour_f');

                    echo json_encode($this->phc_model->get_rep_pickups_mssql($data));
                    break;
                case 'delete_pickup_zone':
                    $data['no'] = $_SESSION["backoffice_user_id"];
                    $data['stamp'] = $this->input->post('stamp');

                    echo $this->product_model->delete_pickup_zone($data);
                    break;
                case 'mud_session':
                    $bostamp = $this->input->post('bostamp');
                    $day = $this->input->post('day');
                    $u_psessstamp = $this->input->post('u_psessstamp');
                    $this->product_model->set_mudar_session($bostamp, $u_psessstamp, $day);
                    break;
                case 'create_pickup_zone':
                    $data['name'] = $this->input->post('name');
                    $data['no'] = $_SESSION["backoffice_user_id"];

                    echo json_encode($this->product_model->create_pickup_zone($data));
                    break;
                case 'create_vouch':
                    $data['input'] = $this->input->post('input');
                ...
                case 'create_product':
                    ...
                    $data['use_r'] = $this->input->post('user');
                    $data['user_2'] = $this->input->post('user2');
                    $data['$user_3'] = $this->input->post('user3');
                ...
                case 'agent_product_manage_fee':
                    ...
                    foreach ($agent_product_fee_tmp as $row) {
                        ...
                        $this->phc_model->update_agent_product_fee($row[0], $agent, $row[4], $row[3]);
                ...
                case 'get_product_session':
                    ...
                    echo json_encode($this->phc_model->get_product_session_mssql($bostamp, $op, 1));
                    break;
                case 'update_extras':
                ...
                case 'update_agent':
                    ...
                    $data['u_locationstamp'] = $this->input->post('u_locationstamp');
                    $data['plafond'] = $this->input->post('plafond');
                    $data['limitevenda'] = $this->input->post('limitevenda');
                    $data['faturaauto'] = $this->input->post('faturaauto');
		    $data['onepage_checkout'] = $this->input->post('onepagecheckout');
                ...
                case 'agent_insert_reimbursement':
                    ...
                    break;
                case 'agent_insert_reimbursement_mb':
                    $data['amount'] = $this->input->post('amount');
                    $data['op'] = $this->input->post('operator');
                    $data['ag'] = $_SESSION["backoffice_user_id"];

                    echo $this->phc_model->agent_insert_reimbursement_mb($data);
                    break;
                case 'process_payment_partial':
                    $data['bostamp'] = $this->input->post('bostamp');
                    $data['issue_invoice'] = $this->input->post('issue_invoice');
                    $data['op'] = $_SESSION["backoffice_user_id"];

                    echo $this->phc_model->process_payment_partial($data);
                    break;
                case 'resend_tickets':
                    $data['bostamp'] = $this->input->post('bostamp');

                    echo $this->phc_model->resend_tickets($data);
                    break;
                case 'clone_product':
                    $bostamp = $this->input->post('bostamp');

                    echo $this->phc_model->clone_product($bostamp);
                    break;
                default:
                    echo 'Nothing to do';
                    break;
