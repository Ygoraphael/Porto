...
class mod_SPSmartSlider {
    ...
    public function generate() {
        ...
        foreach ((array) $params as $index => $class) {
            ...
            if (isset($params[$index]['state']) and $params[$index]['state'] != 'published')
                continue;
    ...
    public function display() {
        ...
        if ($this->params->get('module_cache') === '1') {
            $data = $this->Cache(
                    'sp_smart_slider.json', array($this, 'generate'), array(), (int) $this->params->get('cache_time'), array($this, 'onDataError'), true
            );
        } else {
            $data = $this->generate();
        }
...
//                else
//                    $document->addScript(JURI::root(true) . '/modules/' . $this->module_name . '/assets/jquery.min.js');
//            }
        } else {
