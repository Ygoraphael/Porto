    ...
    <div class="form-grids widget-shadow" data-example-id="basic-forms"> 
        <div class="form-body col-xs-12 col-sm-6">
            <form>
				...
				<div class="form-group">
                    <label>Peso Encomenda</label>
                    <input type="number" id="peso" onfocus="$(this).select()" type="number" min="0" step="0.1" class="decimal_input form-control" value="<?= number_format($enc["u_pesotot"], 2, '.', '') ?>">
                </div>
				<div class="form-group">
                    <label>Volume Encomenda</label>
                    <input type="number" id="volume" onfocus="$(this).select()" type="number" min="0" step="0.1" class="decimal_input form-control" value="<?= number_format($enc["u_volume"], 2, '.', '') ?>">
                </div>
        ...
        <table class="table table-bordered enclinhas">
            <thead>
                <tr>
                    ...
                    <th>Ul.</th>
                    <th>Qtd.</th>
                    <th style="display:none;">Preço Unit.</th>
            ...
            <tbody>
                <?php foreach ($enclin as $lin) { ?>
                    ...
                    <tr>
                        ...
                        <td style="display:none;"><?= number_format($lin["edebito"], 2, '.', ''); ?></td>
                        <td style="display:none;"><?= number_format($lin["desconto"], 2, '.', ''); ?></td>
                        <td style="display:none;"><?= number_format($lin["ettdeb"], 2, '.', ''); ?></td>
...
<script>
    ...
    function saveOrder() {
        jQuery(".loading-overlay").show();
		var bostamp = jQuery("#bo_bostamp").val().toString().trim();
		var peso = parseFloat(jQuery("#peso").val());
		var volume = parseFloat(jQuery("#volume").val());
		...
		jQuery.post('<?= base_url(); ?>' + 'ajax/update_encomenda', {bostamp: bostamp, peso: peso, volume: volume}, function (response) {}, 'json').done(function (response) {});
        jQuery(".loading-overlay").hide();
