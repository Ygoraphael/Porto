...
class Wl_model extends CI_Model {
    ...
    public function get_ppickups_mssql_product($bostamp) {
        $query = "
			select *
    ...
    public function getMeta_tags() {
        $this->db->select("*");
        $this->db->from('seo_meta_tags');
        $query = $this->db->get()->result_array();
        return $query;
    }

    public function getMeta_tags2($data) {
        $query = "
			select 
					u_metatags
			from
    ...
    public function set_wl_var(&$data, $op) {
        ...
        if (sizeof($wl_data) > 0) {
            ...
            $data["paginationactive"] = $wl_data['u_pagact'];
            $data["paginationdefaultnum"] = $wl_data['u_pagnum'];
            $data["cur_page"] = $this->uri->segment(3);
            $data["languages"] = $this->phc_model->get_languages();
            $data["currency"] = $this->phc_model->get_currency();
            $data["op_existe"] = 1;
        } else {
            $data["op_existe"] = 0;
        }
    ...
    public function get_agent_product_percentage_parcial($no, $bostamp) {
        $this->db->select('perparcial');
        $this->db->from('u_pagent');
        $this->db->where('no', $no);
        $this->db->where('bostamp', $bostamp);
        $query = $this->db->get()->result_array();

        if (sizeof($query) > 0)
            return $query[0]["perparcial"];
        else
            return 0;
    }
    ...
    public function get_all_products() {
        $this->db->select("bo.*,bo2.*,bo3.*,IFNULL((select img from u_pimg where u_pimg.bostamp = bo.bostamp LIMIT 1),'') img
		");
        $this->db->from('bo');
    ...
    public function get_all_bo_products_operador($no) {
        $query = "select bostamp from bo where no = {$no} and ndos = 1 union all select bostamp from u_pagent where no = {$no}";
        $query = $this->mssql->mssql__select($query);
        return $query;
    }

    public function getProducts_of_operador($bo) {

        $this->db->select("produto.u_name,produto.*,produto2.*,produto3.*,IFNULL((select img from u_pimg where u_pimg.bostamp = produto.bostamp LIMIT 1),'') img
		");
        $this->db->from('bo produto');
        $this->db->join('bo2 produto2', 'produto.bostamp=produto2.bo2stamp');
        $this->db->join('bo3 produto3', 'produto2.bo2stamp = produto3.bo3stamp');
        $this->db->where('produto.bostamp', $bo);
        $query = $this->db->get()->row_array();
        return $query;
    }

    public function get_filters($operator, $parametros, $bostamps = array(), $disable = 0) {
        ...
        if (sizeof($bostamps) > 0) {
            $this->db->where_in('bo.bostamp', $bostamps);
        }
    ...
    public function get_product_languages($bostamp) {
        $this->db->select('u_lang.language, u_lang.code');
        $this->db->distinct();
        $this->db->from('u_plang');
        $this->db->join('u_lang', 'u_plang.language = u_lang.language');
        $this->db->where('u_plang.bostamp', $bostamp);
        $this->db->order_by('language', 'ASC');
        $query = $this->db->get();
    ...
    public function get_product_languages_mssql($bostamp) {
        $query = "
			select distinct u_lang.language, u_lang.code
			from u_plang
			inner join u_lang on u_plang.language = u_lang.language
    ...
    public function get_products($operator, $filters, $limit = "", $start = "", $disable = 0, $agent) {
    ...
    public function get_products_listing($operator, $filters, $limit = "", $start = "", $disable = 0, $agent) {
        $this->db->limit($limit, $start);
    ...
    public function get_lastminute_products($operator, $filters, $disable = 0) {
        $this->db->select("
		bo.*,
		bo2.*,
		bo3.*,
    ...
    public function get_products_top($operator, $disable = 0, $agent) {
        $this->db->select("
		bo.*,
		bo2.*,
		bo3.*,
    ...
    public function get_category($operator, $disable = 0, $agent) {
        $this->db->select('
    ...
    public function get_topdest($operator, $disable = 0, $agent) {
    ...
    public function get_product_data($operator, $sefurl, $disable = 0) {
        $this->db->select('
		bo.*,
		bo2.*,
		bo3.*
    ...
    public function get_product_data_agencia($sefurl, $disable = 0) {
        $this->db->select('
		bo.*,
		bo2.*,
		bo3.*
    ...
    public function get_product_extras($sefurl, $disable = 0) {
        $this->db->select('
		u_prec.*
		');
        $this->db->from('bo');
    ...
    public function get_product_categories($sefurl, $disable = 0) {
        $this->db->select('
		u_pcateg.*
		');
        $this->db->from('bo');
