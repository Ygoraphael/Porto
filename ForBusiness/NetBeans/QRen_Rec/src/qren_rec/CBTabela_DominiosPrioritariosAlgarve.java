...
class CBTabela_DominiosPrioritariosAlgarve
  extends CBTabela
{
  ...
  CBTabela_DominiosPrioritariosAlgarve()
  {
  ...
  void initialize() {
    tag = "DominiosPrioritariosAlgarve";
    started = true;
    force_xml = true;
    ...
    init_handler(P.jTable_DominiosAlgarve);
    P.jTable_DominiosAlgarve.addKeyListener(new TableKeyListener(this));
    handler.width = P.jScrollPane_DominiosAlgarve.getWidth();
    
    handler.set_col_text(0, 0.05D, "C");
    handler.set_col_comboS(2, 0.3D, null, CTabelas.ENEIDominioAlgarve, 1, 350);
  ...
  void _filter_populate_areas(SteppedComboBox cbo_areas, int row)
  {
    ...
    if (dominio.length() == 0) {
      ...
      CTabelas.ENEIAreaAlgarveF1.set_filter(2, dominio);
      CTabelas.ENEIAreaAlgarveF1._populateComboBox(cbo_areas);
    }
    if (area.length() > 0) {
      int n = CTabelas.ENEIAreaAlgarveF1.getIndexFromCode(area);
  ...
  void on_update(String colname, int nRow, String v) {
    ...
    if (colname.equals("area_d")) {
      ...
      if (v.length() > 0) {
        area = CTabelas.ENEIAreaAlgarveF1.lookup(1, v, 0);
        String dominio_d = CTabelas.ENEIDominioAlgarve.lookup(0, area.substring(0, 2), 1);
    ...
    if (colname.equals("area")) {
      String area_d = "";
      if (v.length() > 0) area_d = CTabelas.ENEIAreaAlgarve.lookup(0, v, 1);
    ...
    if (colname.equals("dominio_d")) {
      ...
      if (v.length() == 0) {
        ...
        dominio = CTabelas.ENEIDominioAlgarve.lookup(1, v, 0);
    ...
    if (colname.equals("dominio")) {
      String dominio_d = "";
      if (v.length() > 0)
        dominio_d = CTabelas.ENEIDominioAlgarve.lookup(0, v, 1);
  ...
  CHValid_Grp validar(CHValid_Grp err_list) {
    if (err_list == null) {
      err_list = new CHValid_Grp(this, "D1. Domínios Prioritários - Algarve");
    }
    if ((isEmpty()) && (_lib.to_double(ParamsgetByName"algarve"v) > 0.0D)) {
