...
class CBTabela_DominiosPrioritariosLisboa
  extends CBTabela
{
  ...
  CBTabela_DominiosPrioritariosLisboa()
  {
  ...
  void initialize() {
    tag = "DominiosPrioritariosLisboa";
    started = true;
    force_xml = true;
    ...
    init_handler(P.jTable_DominiosLisboa);
    P.jTable_DominiosLisboa.addKeyListener(new TableKeyListener(this));
    handler.width = P.jScrollPane_DominiosLisboa.getWidth();
    
    handler.set_col_text(0, 0.05D, "C");
    handler.set_col_comboS(2, 0.3D, null, CTabelas.ENEIDominioLisboa, 1, 350);
  ...
  void _filter_populate_areas(SteppedComboBox cbo_areas, int row)
  {
    ...
    if (dominio.length() == 0) {
      ...
      CTabelas.ENEIAreaLisboaF1.set_filter(2, dominio);
      CTabelas.ENEIAreaLisboaF1._populateComboBox(cbo_areas);
    }
    if (area.length() > 0) {
      int n = CTabelas.ENEIAreaLisboaF1.getIndexFromCode(area);
  ...
  void on_update(String colname, int nRow, String v) {
    ...
    if (colname.equals("area_d")) {
      ...
      if (v.length() > 0) {
        area = CTabelas.ENEIAreaLisboaF1.lookup(1, v, 0);
        String dominio_d = CTabelas.ENEIDominioLisboa.lookup(0, area.substring(0, 2), 1);
    ...
    if (colname.equals("area")) {
      String area_d = "";
      if (v.length() > 0) area_d = CTabelas.ENEIAreaLisboa.lookup(0, v, 1);
    ...
    if (colname.equals("dominio_d")) {
      ...
      if (v.length() == 0) {
        ...
        dominio = CTabelas.ENEIDominioLisboa.lookup(1, v, 0);
    ...
    if (colname.equals("dominio")) {
      String dominio_d = "";
      if (v.length() > 0)
        dominio_d = CTabelas.ENEIDominioLisboa.lookup(0, v, 1);
  ...
  CHValid_Grp validar(CHValid_Grp err_list) {
    if (err_list == null) {
      err_list = new CHValid_Grp(this, "D1. Domínios Prioritários - Lisboa");
    }
    if ((isEmpty()) && (_lib.to_double(ParamsgetByName"lisboa"v) > 0.0D)) {
