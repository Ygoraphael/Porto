...
// echo 'rview = '.$rview;
			...
			if (strpos ($this->fTask, 'checkout') || $this->address_type == 'ST') {
				$buttonclass = 'button button-caution';
			}
			else {
				$buttonclass = 'button button-caution';
			}
			...
			if (VmConfig::get ('oncheckout_show_register', 1) && $this->userId == 0 && $this->address_type == 'BT' and $rview == 'cart') {
				...
				<?php if (!VmConfig::get ('oncheckout_only_registered', 0)) { ?>
					...
					<?php } ?>
				<button class="button button-caution" type="reset"
				...
				<button class="button button-caution" type="button" onclick="javascript:return myValidator(userForm, '<?php echo $this->fTask; ?>');"><?php echo JText::_ ('COM_VIRTUEMART_SAVE'); ?></button>
				<button class="button button-caution" type="reset" onclick="window.location.href='<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=' . $rview); ?>'"><?php echo JText::_ ('COM_VIRTUEMART_CANCEL'); ?></button>
