...
class shopFunctionsF {
	...
	static public function getAddToCartButton ($orderable) {
		...
		if($orderable) {
			$html = '<input type="submit" name="addtocart" class="custom-a custom-btn" value="'.JText::_( 'COM_VIRTUEMART_CART_ADD_TO' ).'" title="'.JText::_( 'COM_VIRTUEMART_CART_ADD_TO' ).'" />';
		} else {
...
// 		$format = (VmConfig::get('order_html_email',1)) ? 'html' : 'raw';
  ...
  AND s.home = 1';
	...
	static function buildTabs ($view, $load_template = array()) {
		...
		vmJsApi::js( 'vmtabs' );
		$html = '<div class="container-fluid"><div class="row"><div class="col-md-12"><div class="drop-shadow col-md-12" id="ui-tabs" style="border:1px solid rgba(0,0,0,0.1);">';
		$i = 1;
		...
		$html .= '</div></div><div class="col-md-1"></div></div>';
		echo $html;
	}
