...
class cfPagination extends JPagination{
	...
	function __construct($total, $limitstart, $limit, $perRow=3){
		...
		if(empty($cfmenus)){
			...
			$this->menuparams->loadString($cfmenus[0]->params);
		}
		...
		else {//get it from the module's settings
	...
	function getVarsArray(){
		...
		foreach($vars as $key=>$val){
			...
			if($key=='virtuemart_category_id' || $key=='virtuemart_manufacturer_id' || $is_custom_filter!==false || $key=='price_from' || $key=='price_to' || $key=='Itemid'){
				if(is_array($val)){
