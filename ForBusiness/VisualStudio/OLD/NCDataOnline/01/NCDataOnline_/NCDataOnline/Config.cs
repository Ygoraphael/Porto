...
namespace NCDataOnline
{
    public class Config
    {
        ...
        public string GetNextFreeSlotId()
        {
            ...
            {
                if (pair.Value["destinoQuery"] == "Slot")
                {
                    int result = -1;
            ...
            foreach (KeyValuePair<string, Dictionary<string, string>> pair2 in this.data.ficheiroJson)
            {
                if (pair2.Value["destinoFicheiro"] == "Slot")
                {
                    int num2 = -1;
                    int.TryParse(pair2.Value["destinoidFicheiro"], out num2);
            ...
            foreach (KeyValuePair<string, Dictionary<string, string>> pair3 in this.data.ficheiroXls)
            {
                if (pair3.Value["destinoFicheiroXls"] == "Slot")
                {
                    int num3 = -1;
                    int.TryParse(pair3.Value["destinoidFicheiroXls"], out num3);
                ...
                if (!(text == "Geral"))
                {
                    if (!(text == "Conexão"))
                    {
                        if (!(text == "Query"))
                        {
                            ...
                            {
                                if ((text == "Ficheiro XLS") && ((((ListBox) form.Controls["listBox5"]).Items.Count > 0) && (((ListBox) form.Controls["listBox5"]).SelectedItem != null)))
                                {
                                    key = ((ListBox) form.Controls["listBox5"]).SelectedItem.ToString();
                                    if (selectedItem != "")
                                    {
                                        key = selectedItem;
                                    }
                                    if (!this.data.ficheiroXls.ContainsKey(key))
                                    {
                                        this.data.ficheiroXls.Add(key, new Dictionary<string, string>());
                                    }
                                    this.data.ficheiroXls[key] = this.ReadFormTo(form);
                                }
                                return;
                            }
                            if ((((ListBox) form.Controls["listBox4"]).Items.Count > 0) && (((ListBox) form.Controls["listBox4"]).SelectedItem != null))
                            {
                                key = ((ListBox) form.Controls["listBox4"]).SelectedItem.ToString();
                                if (selectedItem != "")
                                {
                                    key = selectedItem;
                                }
                                if (!this.data.ficheiroJson.ContainsKey(key))
                                {
                                    this.data.ficheiroJson.Add(key, new Dictionary<string, string>());
                                }
                                this.data.ficheiroJson[key] = this.ReadFormTo(form);
                            }
                            return;
                        }
        ...
        private bool Verify(out string messageText)
        {
            ...
            dictionary4.Add("destinoQuery", "Tipo de destino");
            ...
            foreach (KeyValuePair<string, Dictionary<string, string>> pair4 in this.data.query)
            {
                ...
                {
                    if (pair4.Value["destinoQuery"] == "Slot")
                    {
                    ...
                    {
                        if (list2.Contains(pair4.Value["destinoidQuery"]))
                        {
                            messageText = "Já está a utilizar o widget " + pair4.Value["destinoidQuery"] + " uma vez neste dashboard.";
                            return false;
                        }
                        list2.Add(pair4.Value["destinoidQuery"]);
                    }
                }
            }
            return true;
