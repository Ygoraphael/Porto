...
public class Familia extends Db {
   ...
   {
       this.lastSQL = ""
            + "select distinct stock.usr4 gama, stock.usr5 genero, stock.usr3 linha, stock.u_codproj, stock.u_descproj "
            + "from "
            + "( "
            + "       select "
        ...
        try {
            ...
            while (rs.next()) 
            {
                ...
                familia.ref = rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim();      
                familia.name = toTitleCase(rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim().toLowerCase());
                familia.famipai = "";
                //tmp_array = rs.getString("imgqlook").split("\\\\"); 
                //familia.imgqlook = tmp_array[tmp_array.length - 1].replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familias.add(familia);
                
                //genero
                familia = new Familia( this.con );
                familia.ref = rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familia.ref += "|||" + rs.getString("genero").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familia.name = toTitleCase(rs.getString("genero").replace("\\", "").replace("\"", "").replace("\'", "").trim().toLowerCase());
                familia.famipai = rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                //tmp_array = rs.getString("imgqlook").split("\\\\"); 
                //familia.imgqlook = tmp_array[tmp_array.length - 1].replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familias.add(familia);
                ...
                familia.ref = rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familia.ref += "|||" + rs.getString("genero").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familia.ref += "|||" + rs.getString("linha").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                familia.name = toTitleCase(rs.getString("linha").replace("\\", "").replace("\"", "").replace("\'", "").trim().toLowerCase());
                familia.famipai = rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim() + "|||" + rs.getString("genero").replace("\\", "").replace("\"", "").replace("\'", "").trim();
                ...
                familia.famipai = rs.getString("gama").replace("\\", "").replace("\"", "").replace("\'", "").trim() + "|||" + rs.getString("genero").replace("\\", "").replace("\"", "").replace("\'", "").trim() + "|||" + rs.getString("linha").replace("\\", "").replace("\"", "").replace("\'", "").trim();
