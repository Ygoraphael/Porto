...
class CBTabela_DominiosPrioritariosNorte
  extends CBTabela
{
  ...
  CBTabela_DominiosPrioritariosNorte()
  {
  ...
  void initialize() {
    ...
    init_handler(P.jTable_DominiosNorte);
    P.jTable_DominiosNorte.addKeyListener(new TableKeyListener(this));
    handler.width = P.jScrollPane_DominiosNorte.getWidth();
    
    handler.set_col_text(0, 0.05D, "C");
    handler.set_col_comboS(2, 0.3D, null, CTabelas.ENEIDominioNorte, 1, 350);
  ...
  void on_update(String colname, int nRow, String v) {
    ...
    if (colname.equals("dominio_d")) {
      String dominio = "";
      if (v.length() > 0)
        dominio = CTabelas.ENEIDominioNorte.lookup(1, v, 0);
      setColValue("dominio", nRow, dominio);
    }
    if (colname.equals("dominio")) {
      String dominio_d = "";
      if (v.length() > 0)
        dominio_d = CTabelas.ENEIDominioNorte.lookup(0, v, 1);
  ...
  CHValid_Grp validar(CHValid_Grp err_list) {
    if (err_list == null) {
      err_list = new CHValid_Grp(this, "D1. Domínios Prioritários - Norte");
    }
    if ((isEmpty()) && (_lib.to_double(ParamsgetByName"norte"v) > 0.0D)) {
