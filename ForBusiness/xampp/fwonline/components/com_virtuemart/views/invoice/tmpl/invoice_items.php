...
<?php  } ?>
	...
	<?php
		$db = JFactory::getDbo();
		$sql = "";
		$sql .= "	select B.payment_name ";
		$sql .= "	from e506s_virtuemart_paymentmethods A inner join e506s_virtuemart_paymentmethods_pt_pt B on A.virtuemart_paymentmethod_id = B.virtuemart_paymentmethod_id ";
		$sql .= "	where A.virtuemart_paymentmethod_id = " . $this->orderDetails['details']['BT']->virtuemart_paymentmethod_id;

		$db->setQuery($sql);
		$result = $db->loadRow();

		if( sizeof($result) > 0 ) {
			$payment_name = $result[0];
		}
		else {
			$payment_name = "";
		}
		...
		$sql = "";
		$sql .= "	select B.shipment_name ";
		$sql .= "	from e506s_virtuemart_shipmentmethods A inner join e506s_virtuemart_shipmentmethods_pt_pt B on A.virtuemart_shipmentmethod_id = B.virtuemart_shipmentmethod_id ";
		$sql .= "	where A.virtuemart_shipmentmethod_id = " . $this->orderDetails['details']['BT']->virtuemart_shipmentmethod_id;

		$db->setQuery($sql);
		$result = $db->loadRow();

		if( sizeof($result) > 0 ) {
			$shipment_name = $result[0];
		}
		else {
			$shipment_name = "";
		}
	...
	<tr>
		<td align="right" class="pricePad" colspan="5"><?php echo $shipment_name ?></td>
	...
	<tr>
		<td align="right" class="pricePad" colspan="5"><?php echo $payment_name ?></td>
