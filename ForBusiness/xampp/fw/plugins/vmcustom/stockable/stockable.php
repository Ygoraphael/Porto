	...
	function plgVmOnDisplayProductVariantFE($field,&$row,&$group) {
		...
		foreach ($selects as $keys =>$options) {
			...
			if (!empty($listname)) {
				...
				$soldout = JText::_("NOVOSCANAIS_OUTSTOCK");
				...
				//tiago hack
				$parameterTitle = JTEXT::_($listname);
				if( $parameterTitle == "Tamanho" ) {
					if( in_array("XS", $options) ) {
						$val = "XS";
						if (1 == $i) {
						...
						$val = "XS";
						$option[] = JHTML::_('select.option', '', JText::_( $val ) . ' ( ' . $soldout . ' )', array('disable' => 'disabled'));
						$show_select = true;
					}
				...
				if ($show_select) {
					...
					//tiago hack
					$html .='<div style="width:100%;"><span class="parameter_stockable">'.$parameterTitle.'</span>';
					$html .= JHTML::_('select.genericlist',$option,$optionName,'class="attribute_list no-vm-bind customfield_id_'.$js_suffix.'" style="width:245px !important;"','value','text',reset($options),'selectoptions'.$i,false)."</div>\n";
				}
				else {
					$html .='<input id="'.$keys.'" class="attribute_list" type="hidden" value="'.$val.'" name="'.$optionName.'">';
				}
			}
			$i++;
		}
		...
		jQuery( function($) {
			...
			function recalculate_'.$js_suffix.'(stockableBlockIndex, Opt){
				...
				if ("disableadd" == stockhandle && found_child.in_stock <= 0) {
					...
					$(".addtocart-bar>div").remove();
					$(".addtocart-bar>a.notify").remove();
					$(".addtocart-bar").append(\'<span class="quantity-box"><input type="text" class="quantity-input js-recalculate" name="quantity[]" value="\' + quantity + \'" /></span><span class="quantity-controls js-recalculate"><input type="button" class="quantity-controls quantity-plus" /><input type="button" class="quantity-controls quantity-minus" /></span><span class="addtocart-button "><input type="submit" name="addtocart" style="background:#eb4800;" class="btn btn-primary addtocart-button" value="'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'" title="'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'" /></span><div class="clear"></div>\');
		...
		if ($customPlugin) {
			...
			foreach ($product as $key => $value) {
				if ('virtuemart_product_id' != $key) {
					//tiago loureiro - correcao a erro
