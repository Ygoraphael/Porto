...
class YT_Minify extends JObject
{
	...
	function oneJQuery(){
		...
		preg_match_all($scriptRegex, $body[0], $matches);
		$regString = "/([^\"\'=]+\.(js))[\"\']/i";
		...
		$strPath = "";
		$first = true;
		$jquery_arr = array('jquery.min.js', 'jquery.js');
		foreach($matches[0] as $match){
			...
			if(isset($arrMatchs[1][0])){
				...
				$file_d = str_replace(J_SEPARATOR, "/", JPATH_SITE)."/".$strTemp;

				$arrJQ = '';
				preg_match_all("/jquery-[\s\S]*?.min.js/i", $file_d, $arrJQ);
				if($app->getTemplate(true)->params->get('optimizeMergeFile', 0) == 1 && $app->getTemplate(true)->params->get('optimizeJS', 0) ==1 ){
					$str_repl = '';
				}else{
					$str_repl = '<!-- '.$filePath.' -->';
				}

				if(in_array(basename($file_d), $jquery_arr) || count($arrJQ[0]) > 0){
					if(!file_exists($file_d)){
						$body[0] = str_replace($match, $str_repl, $body[0]);
					}elseif($first){
						$first = false;
					}else{
						$body[0] = str_replace($match, $str_repl, $body[0]);
