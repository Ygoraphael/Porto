...
class modOsTouchSliderHelper{
  ...
  public static function getAjax() {
    ...
    if($task == 'save_settings'){
      ...
      $params->set("object_fit_cover", (isset($frontendParams['object_fit_cover'])&& $frontendParams['object_fit_cover'] != 'false')? $frontendParams['object_fit_cover'] : 0);
  ...
  static function fileImport($importData)
  {
      ...
      foreach($objectImage->node as $image)
      {
          $old_id = clone $image->node[0];
          $image->node[0] = ++$last_id;
          ...
          $image_info->textAnimation->start[(int)$image->node[0]] = $image_info->textAnimation->start[(int)$old_id];
          $image_info->textAnimation->start[(int)$old_id]='';
          $image_info->textAnimation->end[(int)$image->node[0]] = $image_info->textAnimation->end[(int)$old_id];
          $image_info->textAnimation->end[(int)$old_id]='';
          $image_info->imageFullTime[(int)$image->node[0]] = $image_info->imageFullTime[(int)$old_id];
      ...
      foreach($objectImage->node as $image){
        ...
        if(!empty($moduleParams))
        {
          ...
          $image_info->textAnimation->start = self::objToArr($image_info->textAnimation->start);
          $image_info->textAnimation->end = self::objToArr($image_info->textAnimation->end);
          $image_info->imageFullTime = self::objToArr($image_info->imageFullTime);

          $moduleParams->textAnimation->start = self::objToArr($moduleParams->textAnimation->start);
          $moduleParams->textAnimation->end = self::objToArr($moduleParams->textAnimation->end);
          $moduleParams->imageFullTime = self::objToArr($moduleParams->imageFullTime);
          ...
          if(isset($moduleParams->textAnimation->start) && isset($image_info->textAnimation->start))
            @$moduleParams->textAnimation->start[(int)$image->node[0]] = $image_info->textAnimation->start[(int)$old_id];
      
          if(isset($moduleParams->textAnimation->end) && isset($image_info->textAnimation->end)) 
            @$moduleParams->textAnimation->end[(int)$image->node[0]] = $image_info->textAnimation->end[(int)$old_id];

          if(isset($moduleParams->imageFullTime) && isset($image_info->imageFullTime)) 
            @$moduleParams->imageFullTime[(int)$image->node[0]] = $image_info->imageFullTime[(int)$old_id];

          if(isset($moduleParams->imageOrdering)) 
            @$moduleParams->imageOrdering[] = (int)$image->node[0];
