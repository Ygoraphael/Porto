...
//$url = $uri->toString(array('path', 'query', 'fragment'));
    ...
    if ($this->order  ) {
	    ...
	    <h3 class="header"><span><?php echo JText::_('COM_VIRTUEMART_ORDER_ANONYMOUS') ?></span></h3>
...
<?php   }
	...
	<div>
		<h3 class="header"><span><?php echo JText::_('COM_VIRTUEMART_ORDER_CONNECT_FORM'); ?></span></h3>
	</div>
<div class="clear"></div>
<?php } else { ?>
...
<?php }   ?>
        ...
        <p class="floatleft" id="com-form-login-remember">
            <input type="submit" name="Submit" class="button subbutton btn btn-primary" value="<?php echo JText::_('COM_VIRTUEMART_LOGIN') ?>" />
        </p>
        <p class="floatleft" id="com-form-login-remember">
            <?php if (JPluginHelper::isEnabled('system', 'remember')) : ?>
            <label id="remember_text" for="remember"><?php echo $remember_me = JVM_VERSION===1? JText::_('Remember me') : JText::_('JGLOBAL_REMEMBER_ME') ?></label>
            <?php endif; ?>
        </p>		
        <p class="floatleft" id="com-form-login-remember">
            <?php if (JPluginHelper::isEnabled('system', 'remember')) : ?>
