...
class JFormFieldPlugins extends JFormFieldList
{
	...
	protected function getInput()
	{
		...
		if ($plugintype == 'field')
		{
			if ($this->form->getValue("group_id") == 1 && $this->form->getValue("id") > 0)
			{
				$db    = JFactory::getDbo();
				$query = "SELECT id, title FROM #__juform_plugins WHERE id = " . $this->form->getValue("plugin_id", null, 0);
				$db->setQuery($query);
	...
	protected function getOptions()
	{
		...
		$plugintype = $this->element['plugintype'];
		$_options   = JUFormHelper::getPluginOptions($plugintype);
