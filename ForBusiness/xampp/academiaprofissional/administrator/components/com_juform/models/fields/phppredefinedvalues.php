...
class JFormFieldPHPPredefinedValues extends JFormField
{
	...
	{
		$JEditor = JEditor::getInstance('codemirror');
		...
		$html .= '</div>';
		if ($this->checkEditor('codemirror'))
		{
			$html .= $JEditor->display($this->name, htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8'), $this->element['width'],
		...
		{
			$script .= '
				var	php_code = ' . $JEditor->getContent('jform_php_predefined_values');
		}
		else
		{
			$script .= '
		...
		$script .= '
				$.ajax({
						type: "POST",
						url: "index.php?option=com_juform&task=field.testPhpCode",
					...
					.done(function( data ) {
						...
						if(data.indexOf("<b>Fatal error</b>") !== -1 || data.match(/Parse error:\s*syntax error,/))
						{
