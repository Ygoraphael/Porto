<?php
/**
 * @version $Id: template.php 153 2016-10-18 10:58:14Z szymon $
 * @package JMFramework
...
abstract class JMFTemplate {
    ...
    public function renderModulesSlider($position, $chrome = 'none', $grid_layout = 12) {
    ...
    public function renderModulesSliderAUX($position, $chrome = 'none', $grid_layout = 12) {
		...
		if ($modules = JModuleHelper::getModules( $posname )) {
			...
			for ($i = 0; $i < $count; $i++) {
				...
				$module_content = $renderer->render($modules[$i], null, null);
	...
	public function renderFlexiblock($name, $chrome = 'none', $cols = 4, $grid_layout = 12) {
		...
		foreach($splparams as $i => $splparam) {
			...
			$html.= '<div class="footerItems '.$param->default.'" '.$datas[$i].'>'; // AQUI AHAHAH BLBLBLBLBLBL AHAHAH
