...
class JDocumentRendererHtmlHead extends JDocumentRenderer {
    ...
    public function fetchHead($document) {
		...
		if( isset($item) ) {
			$params = $item->params;
			if( !$document->_metaTags["name"]["keywords"] && $params->get('menu-meta_keywords') ) {
				$buffer .= $tab . '<meta name="keywords" content="' . htmlspecialchars($params->get('menu-meta_keywords'), ENT_COMPAT, 'UTF-8') . '" />' . $lnEnd;
				$system_meta_tags = 1;
			}
		}
		if( !$system_meta_tags ) {
		...
		if( isset($item) ) {
			$params = $item->params;
			if( !$documentDescription && $params->get('menu-meta_description') ) {
				$documentDescription = $params->get('menu-meta_description');
			}
		}
		//TIAGO HACK		
        ...
        //TIAGO HACK
        $robots_found = 0;
        if (array_key_exists('name', $document->_metaTags)) {
            if (array_key_exists('robots', $document->_metaTags["name"])) {
                $robots_found = 1;
            }
        }
        if (!$robots_found) {
            ...
            if (isset($item)) {
                ...
                if ($params->get('robots')) {
                    $buffer .= $tab . '<meta name="robots" content="' . $params->get('robots') . '" />' . $lnEnd;
                }
            }
        }
        //TIAGO HACK
