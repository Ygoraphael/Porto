...
// 		vmdebug('$sessionCart',$sessionCart);
		...
		if ($result1 != null){
			$profile->data = serialize($sessionCart);
			$profile->userid = $user->id;
			$result = JFactory::getDbo()->updateObject('session_user', $profile, 'userid');
		}else{
			$profile->data = serialize($sessionCart);
			$profile->userid = $user->id;
			$result = JFactory::getDbo()->insertObject('session_user', $profile);
		}
		/* TIAGO - FIM*/
