...
// 		vmdebug('my $this->searchcustoms ',$this->searchcustoms);
			...
			if ((isset($isSite) && $isSite) || $app->isSite()) {
				...
				// ----------------------------------
				$cp_helper_path = JPATH_BASE .'/modules/mod_vm_cherry_picker/helpers/vmFilterProcessor.php';
				$cp_helper = 'VMFilterProcessorPT';
				$cp_installed = true;
				if (! file_exists($cp_helper_path)) {
					$cp_helper_path = JPATH_BASE .'/modules/mod_cherry_picker/helpers/virtuemart.php';
					if (! file_exists($cp_helper_path))
						$cp_installed = false;
					$cp_helper = 'CPPTVirtuemartHelper';
				}

				if ($cp_installed) {
					require_once($cp_helper_path);
					$prices = $cp_helper::getPriceSelection();
					if ($prices) {
					...
					$filtersData = $cp_helper::getFilterSelection();
					if ($filtersData) {
					...
					if (method_exists($cp_helper, 'getManufacturersSelection')) {
						$manufacturers = $cp_helper::getManufacturersSelection();
						if ($manufacturers) {
							$where[] = $manufacturers;
							$joinMf = true;
				...
				// ----------------------------------
				$cp_helper_path = JPATH_BASE .'/modules/mod_vm_cherry_picker_cf/helpers/vmFilterProcessor.php';
				$cp_helper = 'VMFilterProcessorCF';
				$cp_installed = true;
				if (! file_exists($cp_helper_path)) {
					$cp_helper_path = JPATH_BASE .'/modules/mod_cherry_picker_cf/helpers/virtuemart.php';
					if (! file_exists($cp_helper_path))
						$cp_installed = false;
					$cp_helper = 'CPCFVirtuemartHelper';
				}

				if ($cp_installed) {
					require_once($cp_helper_path);
					// If there are CF and PT versions of CP on page, process prices just once
					if ( !$cp_added_prices && ($prices = $cp_helper::getPriceSelection())) {
						// New, more sophisticated way to search by prices
						if (is_array($prices)) {
							$additionalTableJoins .= ' '. $prices['join'];
							$where[] = $prices['where'];
					...
					$join = $cp_helper::getFilterSelection();
					if ($join)
					...
					if ($useDefaultVMManufacturers &&
						method_exists($cp_helper, 'getManufacturersSelection')) {
						$manufacturers = $cp_helper::getManufacturersSelection();
						if ($manufacturers) {
							$where[] = $manufacturers;
							$joinMf = true;
		...
		//tiago hack
		$product_ids2 = $this->TodosOsids (2, $select, $joinedTables, $whereString, $groupBy, $orderBy, $this->filter_order_Dir, $nbrReturnProducts);
		//se com filtro
		//remover artigos que não pertencem ao filtro
		if(!empty($product_ids2)) {
			$jinput = JFactory::getApplication()->input;
			$catfilter = $jinput->get('catfilter', '');
			...
			if( !empty($catfilter) ) {
				...
				$sql = "select virtuemart_category_id
						...
						where phc_ref in ('MARCAS', 'PRODUTOS')";
				...
				foreach( $result as $row ) {
					$fam_topo[] = $row->virtuemart_category_id;
				}
				
				$tipo = $this->getTipoCat2();
				
				foreach ($product_ids2 as $key=>$value) {
					if( !$this->CatPertenceFiltro($catfilter, $value, $fam_topo, $tipo) ) {
						unset($product_ids2[$key]);
					}
				}
			}
		}
		
		$product_ids = $this->exeSortSearchListQuery (2, $select, $joinedTables, $whereString, $groupBy, $orderBy, $this->filter_order_Dir, $nbrReturnProducts, $product_ids2);
		//tiago hack filtro
	...
	//tiago hack filtro
	function getTipoCat2() {
		$db = JFactory::getDbo();
		$cur_cat_id = JRequest::getInt('virtuemart_category_id');
		$language = JFactory::getLanguage();
		
		$query = $db->getQuery(true);
		$sql = "select category_parent_id
				from #__virtuemart_category_categories
				where category_child_id = " . $cur_cat_id;
		$db->setQuery($sql);
		$result = $db->loadObjectList();
		...
		if( sizeof($result) > 0 ) {
			...
			$tmp_id = $result[0]->category_parent_id;
			
			while( $tmp_id != 0 ) {
				if( $cur_loop > $limit_loop ) {
					$tmp_id_last = 0;
					$tmp_id = 0;
					break;
				}
				...
				$sql = "select category_parent_id
						from #__virtuemart_category_categories
						where category_child_id = " . $tmp_id;
				$db->setQuery($sql);
				$result = $db->loadObjectList();
		...
		$sql = "
				...
				from #__virtuemart_categories_pt_pt
				where virtuemart_category_id = " . $tmp_id_last;
		$db->setQuery($sql);	
		$result = $db->loadObjectList();
		
		$tipo = $result[0]->category_name;
		
		return $tipo;
	}

	function TipoFam2($tmp_id, $fam_topo) {
		$db = JFactory::getDbo();
		$language = JFactory::getLanguage();
		...
		while( !in_array($tmp_id, $fam_topo) ) {
		...
		$sql = "select slug
				from #__virtuemart_categories_" . strtolower(str_replace("-", "_", $language->getTag())) . "
				where virtuemart_category_id = " . $tmp_id_last;
		$db->setQuery($sql);
		$result = $db->loadObjectList();
		...
		$cur_slug = '';
		if( sizeof($result) > 0 ) {
			$cur_slug = $result[0]->slug;
		}
		
		return $cur_slug;
	}

	function CatPertenceFiltro($filtro, $idProduto, $famTopo, $tipo) {
		$db = JFactory::getDbo();
		$language = JFactory::getLanguage();
		
		$sql  = "select distinct A.virtuemart_category_id ";
		$sql .= "from #__virtuemart_product_categories A ";
		$sql .= "inner join #__virtuemart_categories B on A.virtuemart_category_id = B.virtuemart_category_id ";
		$sql .= "where A.virtuemart_product_id = " . $idProduto . " and B.published = 1 ";
		
		if( $tipo == "Marcas" ) {
			$sql .= "and A.ordering = 1 ";
		}
		else {
			$sql .= "and A.ordering = 0 ";
		}
		...
		if( $num_rows > 0 ) {
			foreach( $result as $row ) {
				$cur_cat_slug_tmp = $this->TipoFam2($row->virtuemart_category_id, $famTopo);
				if(!in_array($cur_cat_slug_tmp, $topo_familias)) {
					$topo_familias[] = $cur_cat_slug_tmp;
				}
			}
		}
		
		if( sizeof($topo_familias)>0 ) {
			if( $topo_familias[0] == $filtro ) return 1;
			else return 0;
		}
		else {
			return 0;
...
// 		return false;
	...
	function getOrderByList ($virtuemart_category_id = FALSE) {
		...
		if (count ($fields) > 1) {
			...
			//$orderByLink = '<ul class="orderlist">';
			...
			foreach ($fields as $field) {
				if ($field != $orderby) {
					...
					//$orderByLink .= '<li style="width:150px;"><a title="' . $text . '" href="' . $link . '">' . $text . '</a></li>';
					$orderByLink .= '<option url="'.$link.'">' . $text . '</option>';
				}
			}
			//$orderByLink .= '</div>';
			//$orderByLink .= '</ul>';
		...
		$orderByList = '
			<label class="left">' . JText::_ ('COM_VIRTUEMART_ORDERBY') . ':</label>
					<select class="btn ordererlist" onchange="GoToUrl(jQuery(this))">
						<option value="" selected="selected">' . JText::_ ('COM_VIRTUEMART_' . $orderby) . ' ' . $orderDirTxt . '</option>
		';
		...
		if( strpos($link, 'dirDesc') ) {
			$orderByList .=	'	<a class="button-arrow button-desc left clearfix" href="' . $link . '" title=""></a>';
		}
		else {
			$orderByList .=	'	<a class="button-arrow button-asc left clearfix" href="' . $link . '" title=""></a>';
		}
		...
		<script>
			function GoToUrl(obj) {
				if( obj.val() ){
					window.location.href =  obj.find("option").filter(":selected").attr("url") ;
				}
			}
		</script>
		';
		...
		//$orderByList = '<div class="orderlistcontainer">
		//	<div class="title">' . JText::_ ('COM_VIRTUEMART_ORDERBY') . '</div>
		//		<div class="activeOrder">
		//			<a title="' . $orderDirTxt . '" href="' . $link . '">' . JText::_ ('COM_VIRTUEMART_' . $orderby) . ' ' . $orderDirTxt . '</a>
		//		</div>';
