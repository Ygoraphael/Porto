...
class JomGeniusClassContent extends JomGeniusParent {
	...
	function __construct() {
		...
		$this->section_id	= $jinput->get('section_id');
	...
	function info( $type ) {
		...
		if ( $type == 'article_tags' or $type == 'articletags' ) {
			return $this->tagsForType( 'com_content.article', $this->info( 'article_id' ) );
		}

		if ( $type == 'category_tags' or $type == 'categorytags' ) {
			return $this->tagsForType( 'com_content.category', $this->info( 'category_id' ) );
		}
	...
	function pageNumber() {		
		...
		if ( $this->view == 'article' ) {
			return $jinput->get( 'limitstart', 0, 'INT' );
		}
		$limitstart = $jinput->get( 'limitstart', 0, 'INT' );
		$limit = $jinput->get( 'limit', 0, 'INT' ); // provided by Joomla, not in the URL itself. # items per page.
	...
	function categoryId() {
		...
		if ( $this->view == "category" or $this->view == "categories" ) {
			...
			$category_id = (int)(JFactory::getApplication()->input->get("catid",0, 'INT'));
		}
