...
if ($options)
{
	...
	if (!$number_columns)
	{
		foreach ($options AS $key => $option)
		{
			...
			$html .= "<label class=\"checkbox-inline\" for=\"" . $this->getId() . '_' . $option->value . "\">$input $text</label>";
		}
	}
	else
	{
		...
		foreach ($options AS $key => $option)
		{
			...
			$html .= "<label class=\"checkbox\" for=\"" . $this->getId() . '_' . $option->value . "\">";
			if ($key == 0)
			{
				$html .= "<input id=\"" . $this->getId() . '_' . $option->value . "\" name=\"" . $this->getName() . "\" " . $this->getAttribute(null, null, "input") . "
						" . $this->getValidateData() . "/>";
			}
			else
			{
