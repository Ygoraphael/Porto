...
<div class="col-lg-12 noleftrightmargin" id="seat_buy">
	<div class="col-lg-12 calendar-top-container noleftrightmargin">
		...
		<div class="calendar-title3 text-center">
			<h3 class="card-header"><?php echo $this->googletranslate->translate($_SESSION["language_code"], 'Select seats'); ?></h3>
		</div>
		
		<div class="col-lg-12 calendar-price noleftrightmargin" style="margin: 25px 0px 15px; ">
			<div id="seat-map" class="seatCharts-container" style="margin-top: 25px" tabindex="0" aria-activedescendant="9_1">
				<?php
					$num_lug_max = 0;
		...
		<div class="col-lg-12 noleftrightmargin">
			<div class="col-lg-12">
				<div id="seat-tickets" class="seat-tickets-container">
					...
					?>
							<div class="form-group">
								<label for="category" class="col-xs-12 col-sm-4 control-label"><?php echo $extra["design"]; ?> <small>(<i class="fa"><?php echo $_SESSION["i"];?></i><?php echo number_format($extra["price"], 2, '.', ''); ?>)</small></label>
					...
					<?php
						...
						?>
							<div class="form-group">
								<label for="pickup" class="col-sm-4 col-xs-12 control-label" style="padding-top: 9px;"><?php echo $this->googletranslate->translate($_SESSION["language_code"], 'PICKUP'); ?></label>
								<div class="col-xs-12 col-sm-8">
							...
							<div class="form-group">
								<label for="pickup" class="col-xs-12 col-sm-4 control-label" style="padding-top: 9px;"><?php echo $this->googletranslate->translate($_SESSION["language_code"], 'ROOM'); ?></label>
								<div class="col-xs-12 col-sm-8">
									<div class="input-group col-xs-12">
						...
						?>
							<div class="form-group">
								...
								<div class="col-sm-8">
									<div class="input-group col-xs-12">
										<select class="form-control col-xs-12" id="language">
		...
		<script>
			...
			function checkout() {
				...
				if(numItems >= u_qttmin){
					...
					{
						"message": "<?php echo $this->googletranslate->translate($_SESSION["language_code"], 'You have to order a minimum limit of'); ?> "+u_qttmin+" <?php echo $this->googletranslate->translate($_SESSION["language_code"], 'tickets for this product.'); ?>",
						"priority": 'error'
					}
					]);
				}
			...
			$('.input-number').change(function() {
				...
				if(valueCurrent >= minValue) {
					...
					alert('<?php echo $this->googletranslate->translate($_SESSION["language_code"], 'Sorry, the minimum quantity was reached'); ?>');
					$(this).val($(this).data('oldValue'));
				}
				if(valueCurrent <= maxValue) {
					$(".btn-number[data-type='plus'][data-field='"+name+"']").removeAttr('disabled')
				} else {
					alert('<?php echo $this->googletranslate->translate($_SESSION["language_code"], 'Sorry, the maximum quantity was reached'); ?>');
					$(this).val($(this).data('oldValue'));
				}
			});
			...
			function update_prices_seats() {
				...
				jQuery.each( jQuery( ".calendar-extra-picker" ), function( i, val ) {
					...
					{
						html+= '<div class="price-category clearfix"><span class="price-cat pull-left">' + jQuery(this).attr("design") + '</span><span class="price-qtt pull-left">' + jQuery(this).val() + ' x <i class="fa">'+currency+'</i> ' + jQuery(this).attr("price") + '</span>';
				...
				$.ajax({
					...
					success : function(data){
						try
						{
							...
							var html = '<select onchange="jQuery(\'.calendar-but-2\').css(\'display\', \'inline\');" class="calendar-time-picker clearfix form-control"><option value="" disabled="" selected=""><?php echo $this->googletranslate->translate($_SESSION["language_code"], 'Select a starting time'); ?></option>';
							if(parse.length>0)
							{
								for(p in parse)
								{
			...
			function calendar_page(page) {
				if( page == 1 ) {
					...
					jQuery(".seat-tickets-container").css("display", "none");
				}
				else if( page == 2 ) {
					//carregar sessoes
					update_hours();
					...
					jQuery(".calendar-but-3").css("display", "none");
					jQuery(".calendar-total").css("display", "none");
					jQuery(".seat-tickets-container").find("form").html("");
					jQuery(".seat-tickets-container").css("display", "none");
				}
				else if( page == 3 ) {
