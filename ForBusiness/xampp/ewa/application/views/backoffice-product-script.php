...
<script>
    ...
    function clone_product(stamp) {
        $(".loading-overlay").show();
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>backoffice/ajax/clone_product",
            data: {
                "bostamp": stamp
            },
            success: function (data)
            {
                if (data) {
                    ...
                    bootbox.alert("Product cloned successfully!");
                } else {
                    $(".loading-overlay").hide();
                    bootbox.alert("Error cloning product!");
                }
            }
        });
    }
...
?>
        ...
        $(function () {
            $("input, textarea, select, #exclusion_table_wrapper, #seats_table").on('change keyup paste', function () {
                bt_save_cancel();
            });
        });

        $(function () {
            $(document).on('change keyup paste', 'input, textarea, select, #exclusion_table_wrapper, #seats_table', function () {
                bt_save_cancel();
            });
        });
            ...
            if (s_t.data().length > 0) {
                jQuery("tbody tr", $table).each(function () {
                    ...
                    scheduling_table_tmp.push(jQuery(this).children().eq(5).find("input").val());
                     //limit hour
                    scheduling_table_tmp.push(jQuery(this).children().eq(13).find("input").val());
                    ...
                    if (data["update_bo"] == 1) {
                        ...
                        data["updated_scheduling_pricing"].forEach(function (entry) {
                            ...
                            jQuery("tbody tr", $table).each(function () {
                                if (jQuery(this).children().eq(0).html() == entry["id"]) {
                                    if (entry["alt_price"]) {
                                        jQuery(this).children().eq(15).html('<a href="<?php echo base_url() . "backoffice/product_price/" . $product["u_sefurl"] . "/" ?>' + entry["id"] + '" class="btn btn-default"><span class="glyphicon glyphicon-euro"></span></a>');
                                    } else {
                                        jQuery(this).children().eq(15).html("");
                                    }
                                }
                            });
                        });
