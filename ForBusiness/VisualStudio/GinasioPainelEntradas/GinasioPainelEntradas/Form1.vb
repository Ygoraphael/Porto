    ...
    Private Sub VerSeExisteCliente(NCartao As String)
        ...
        Try
            ...
            If sqlReader.HasRows Then
                ...
                cnn.Close()
                cnn.Open()
                command = New SqlCommand("select dataopen, tabela1 From bo Where ndos = 1 And fechada = 0 And dateadd(year, 1, dataobra) >= getdate() And no=" & Val(NumeroCliente), cnn)
    ...
    Private Sub VerSeExisteClienteNumero(NCartao As String)
        ...
        Try
            ...
            If sqlReader.HasRows Then
                ...
                cnn.Close()
                cnn.Open()
                command = New SqlCommand("select dataopen, tabela1 From bo Where ndos = 1 And fechada = 0 And dateadd(year, 1, dataobra) >= getdate() And no=" & Val(NCartao), cnn)
                sqlReader = command.ExecuteReader()
                If sqlReader.HasRows Then
                    sqlReader.Read()
