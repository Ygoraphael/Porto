...
if (!class_exists ('vmPSPlugin')) {
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		$html = '<table class="vmorder-done table" align="center">' . "\n";
		...
		//tiago hack
		preg_match('/<span[^>]*>(.*?)<\/span>/i', $cart->cartData["shipmentName"], $matches);
		$html .= $this->getHtmlRow ('METODOENVIO', $matches[1], 'class="vmorder-done-amount"');
		$MoradaEnvio = trim($cart->ST["address_type_name"]);
		if( $MoradaEnvio == '' )
			$html .= $this->getHtmlRow ('ENDERECOENVIO', JText::_('ENDERECOPADRAO'), 'class="vmorder-done-amount"');
		else
			$html .= $this->getHtmlRow ('ENDERECOENVIO', $MoradaEnvio, 'class="vmorder-done-amount"');
		//tiago hack
		...
		vRequest::setVar ('old_order_number', $order['details']['BT']->order_number);
		return TRUE;
	}
