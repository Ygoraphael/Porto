...
/**
* @version		3.0
* @copyright	Copyright (C) 2007-2015 Stephen Brandon
* @license		GNU/GPL
*/
...
class modMetaModHelper
{
	...
	public static function versionMajor() { return 3; }
	public static function versionMinor() { return '21'; }
	
	static function locateGeoIPInclude($use_geoip) {
		...
		foreach ($geoip_folders as $folder) {
			...
			$proposed_file = JPATH_SITE.'/'.$folder.'/'.$target;
	...
	public static function move_to_end_of_array( $key, &$array ) {
	...
	public static function advanced_debug() {
		...
		if ( version_compare( JVERSION, '3.0', '>=' ) ) { // >= J3.0
			...
			//$get_keys = array_keys($gets);
			$cookie_keys = array_keys($_COOKIE);
		
			// remove cookies from request array. Bit of a problem if the cookies override a value from somewhere else. But I don't know how to access ONLY the parameters from the router!
			foreach ($cookie_keys as $k) {
				if (array_key_exists($k, $requests)) {
					unset($requests[$k]);
				}
			}
...
//				$val = str_replace("'", '\\\'', $val); // escape single quotes
				...
				if ($get == 'Itemid' or $get == 'option' or $get == 'view' or $get == 'id') {
					...
					echo "\$jinput->get({$constant}'" . htmlentities( $get, ENT_QUOTES, 'UTF-8' ) . "'{$color_end}) ";
				}
	...
	public static function timezone_offset( $tz ) {
	...
	public static function getUserIP( $params ) {
	...
	static function geoip_cache_ip_countryid_name_city( $ip, $country_id = null, $country_name = null, $city = null ) {
		global $geoip_cache;
	...
	//           its own position)
	public static function moduleIdsAndChanges( &$params, $my_module = null )
	{
		$jinput				= JFactory::getApplication()->input;
		...
		if ($timezone == "default") {
			...
			if ( version_compare( JVERSION, '3.0', '<' ) ) { // J1.7, 2.5
				$timezone		= $config->getValue('offset');// j3.0 was getValue
			} else {
				$timezone		= $config->get('offset');// j3.0 was getValue
			}
		}
			...
			if ($fromCountryId == '' and $fromCountryName == '' and $use_geoip == 1 ) {
				...
				if ($include_file == null) {
					...
					if (!function_exists("geoip_open")) {
						include_once(JPATH_SITE . '/modules/mod_metamod/geoip/' . $geoIPFiles[$use_geoip]);
					}
		...
		foreach ($user_types as $u) {
			...
			if (!defined($constant))		define( $constant, is_array($user->groups) ? (array_key_exists( $u, $user->groups)) : false );
			if (!defined($constant_not))	define( $constant_not, is_array($user->groups) ? (!array_key_exists( $u, $user->groups)) : true );
		}
		...
		include_once dirname(__FILE__) . '/JomGenius.class.php';
	...
	public static function quotedEscapedListFromModuleArray( &$db, $all_mod_array ) {
		return "'". implode( "','", array_map( array( $db, 'escape'), $all_mod_array ) ) . "'"; //J3.0 changed getEscaped to escape
	}
		...
		if (is_array($all_mod_array) && count($all_mod_array)) {
			...
			if ( version_compare( JVERSION, '3.0', '<' ) ) { // J1.7, 2.5
				if ( !( $modules = $db->loadObjectList() ) ) {
					if ( $debug ) JError::raiseWarning( 'SOME_ERROR_CODE', "Error loading Modules: " . $db->getErrorMsg());				
				...
				{
					if ( $debug ) JLog::add(JText::sprintf('JLIB_APPLICATION_ERROR_MODULE_LOAD', $e->getMessage()), JLog::WARNING, 'jerror');
					return false;
				}
			}
			...
			} else { // J1.7-2.5; get the parameter named "style"
	 ...
	 */
	public static function applyParamChanges( &$modules, &$changeObject ) {
