...
<body>
		...
		<div class="container-fluid-full">
			<div class="row-fluid">
				...
				<div id="content" class="span10">
					<?php 
						$current_page = "Encomenda";
						include("breadcrumbs.php"); 
						$info = get_doc_data($_GET['bostamp'], 'bo');
					?>
					...
					<div class="row-fluid">
						<div class="form-horizontal">
							...
							<button type="submit" class="btn btn-primary" onclick="reEncomendar('<?php echo $_GET['bostamp']; ?>'); return false;" ><i class="white halflings-icon refresh"></i> Re-encomendar</button>
							<br><br>
							<h1><?php echo $info["nmdos"]; ?> nº<?php echo $info["obrano"]; ?></h1>
						</div>
						<div class="span5">
							<form class="form-horizontal">
								...
								<div class="controls">
									<input disabled type="text" class="span3" value='<?php echo substr($info["dataobra"], 0, 10); ?>'>
								</div><br>
								<label class="control-label">Estado</label>
								<div class="controls">
									<input disabled type="text" class="span3" value='<?php if( $info["fechada"] == "1" ) { echo "Fechado"; } else { echo "Em Aberto"; } ?>'>
								</div><br>
							</form>
						</div>
						<div class="span5">
					...
					<div class="row-fluid">
						<form class="form-horizontal">
							...
							<div class="controls">
								<input disabled type="text" class="span2" value='<?php echo number_format($info["etotaldeb"], 2); ?>'>
							</div><br>
						</form>
					</div>
				</div>
	...
	<script>
		$(document).ready(function () { 
			...
			<?php
				$dados = get_doc_data_l($_GET['bostamp'], 'bi', 'bo');
				...
				foreach($dados as $row) {
					...
					echo "dados_tmp.push('" . number_format($row["edebito"],2) . "');";
					...
					echo "dados_tmp.push('" . number_format($row["ettdeb"],2) . "');";
		...
		function reEncomendar(stamp) {
			jQuery.ajax({
				type: "POST",
				url: "funcoes_gerais.php",
				data: { "action" : "re_encomenda('"+stamp+"', <?php echo $_SESSION["user"]["id"]; ?>);"},
				success: function(data) 
				{
					...
					if( successed_order ) {
						location.replace("novaencomenda.php?r=<?php echo $_SESSION["user"]["id"]; ?>");
					}
					else {
						bootbox.alert("Não foi possivel re-encomendar esta encomenda! Tente novamente mais tarde.");
					}
