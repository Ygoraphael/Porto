...
echo '<div class="cp-filter-checkboxlist disable-select" id="cpFilters'. $moduleID .'">';
...
foreach ($filtersCollection as $i => $productType) {
	...
	foreach ($productType as $j => $parameter) {
		...
		//spanish filters
		if ( $parameterTitle == "Preço" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Precio";
		}if ( $parameterTitle == "Cor" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Color";
		}
		if ( $parameterTitle == "Tamanho" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Tamaño";
		}
		if ( $parameterTitle == "Modalidade" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Deporte";
		}
		if ( $parameterTitle == "Género" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Género";
		}
		if ( $parameterTitle == "Linha" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Línea";
		}
		if ( $parameterTitle == "Categoria" and $lang->getTag() == "es-ES" ) {
			$parameterTitle = "Categoría";
		}

		//tiago hack & ivo hack :P

		echo '</h2><div class="btn btn-primary no-border-radius accordion" style="width:100%;margin-left:6%;">
		<div class="sign"></div>
		 <span class="cp-chkb-group-title" style="color:white !important; font-family:raleway, sans-serif;">'. JText::_($parameterTitle) .'</span> </div>';
		...
		foreach ($parameter['filters'] as $k => $filter) {
			...
			echo '<li'. $qrFilterParentClass .'><div class="row"><input id="cp'. $moduleID .'_inpt_'. $parameterName .'_'. $k .'" type="checkbox" value="'.
			...
			if ( $lang->getTag() == "en-GB" ) {
				...
				if( $num_rows > 0 ) {
					...
					echo '<label for="cp'. $moduleID .'_inpt_'. $parameterName .'_'. $k .'" class="cp-filter-label">' . '<span'. $qrFilterClass . $dataFilter .'>teste'. $filterName .'</span>';
				}
			}
			else {
...
if ($manufacturersCollection) {
	...
	foreach ($manufacturersCollection as $i => $mf_category) {
		...
		foreach ($mf_category['mfs'] as $k => $mf) {
			...
			echo '<li'. $qrFilterParentClass .'>
			<input id="cp'. $moduleID .'_inpt_'. $manufacturerCategorySlug .'_'. $k .'" type="checkbox" value="'. htmlentities($mf['slug'], ENT_QUOTES, "UTF-8") .'"'. $dataFilter .' class="cp-filter-input" data-groupname="'. $manufacturerCategorySlug .'"'. $checked .' />';
...
if (!($conf->get('enable_dynamic_update') && $conf->get('update_each_step'))) {
	...
	<button type="submit" style="text-align:center; font-size:19px; border:none; cursor:pointer; width:70% !important;" class="btn btn-primary no-border-radius">
	<span>'. JText::_($conf->get('apply_filters')) .'</span>
	</button></div>';
}
