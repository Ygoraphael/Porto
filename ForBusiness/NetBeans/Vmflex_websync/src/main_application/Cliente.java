...
public class Cliente extends Db {
    ...
    public ArrayList<Cliente> list(Date data) throws ParseException, UnsupportedEncodingException {
        ...
        this.lastSQL
                ...
                + "REPLACE(u_usernm, char(34), '') username, "
                + "REPLACE(email, char(254), '') email, "
                + "REPLACE(fax, char(34), '') fax, "
                + "REPLACE(u_userpw, char(34), '') password, "
        ...
        try {
            ...
            while (rs.next()) {
                ...
                cliente.name = this.toBase64(rs.getString("name"));
                cliente.ncont = rs.getString("ncont");
                cliente.address = this.toBase64(rs.getString("address"));
                cliente.city = this.toBase64(rs.getString("city"));
                cliente.zip = this.toBase64(rs.getString("zip"));
                cliente.phone = rs.getString("phone");
                cliente.email = rs.getString("email");
                cliente.fax = rs.getString("fax");
                cliente.username = this.toBase64(rs.getString("username"));
                cliente.password = this.toBase64(rs.getString("password"));
