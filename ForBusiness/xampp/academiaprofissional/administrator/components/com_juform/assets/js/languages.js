...
jQuery(document).ready(function ($) {
	...
	$(document).on('click', '.remove-language', function () {
		...
		if (stt == true) {
			...
			$.ajax({
				type : "POST",
				url  : "index.php?option=com_juform&task=languages.removeAjax",
	...
	$(document).on('click', '.save-language-new, .save-language', function () {
		...
		if ($(this).hasClass('save-language')) {
			...
			originalValue = (lang != 'en-GB') ? parent.prev().text() : '';
		} else {
		...
		if (inputValue != '' && key != '') {
			$.ajax({
				type : "POST",
				url  : "index.php?option=com_juform&task=languages.saveAjax",
				...
				if (data['success']) {
					...
					html += '<input type="text" name="' + key + '" class="input-xlarge"  size="65" value="" /> ';
					...
					parent.parent('.add').removeClass('add').addClass('new');
					parent.parent().html(html).find('[name="'+key+'"]').val(inputValue);
				} else {
					alert(data['message']);
				}
