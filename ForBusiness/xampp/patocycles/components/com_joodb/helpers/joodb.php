...
class JoodbHelper
{
    ...
    static function parseTemplate(&$joobase, & $parts, &$item) {
        ...
        foreach( $parts as &$part ) {
            if ($doOutput) {
                ...
                } else if ($part->function=="editbutton") { // insert edit button
                    $output .= self::getEditButton($item,$joobase,$part);
                } else if ($part->function=="deletebutton") { // insert delete icon
                    $output .= self::getDeleteButton($item,$joobase);
                } else if ($part->function=="backbutton") { // insert backlink
                    $output .= self::getBackbutton();
                } else if ($part->function=="translate") { // Translate through joomla Language system
    ...
    static function replaceField(&$joobase, &$part, $field, $itemlink,$id) {
        ...
        switch($function) {
            ...
            case "mediumtext":
            case "longtext":
                if (($function=="varchar" || $function=="tinytext") && $params->get('link_urls','0')) {
            ...
            if ($fieldname==$field->Field) {
                ...
                switch ($typearr[0]) {
                    ...
                    case 'longtext' :
                        ...
                        $formfield = $editor->display($fieldname, stripslashes($value), '45ß', '250', '40', '6',false,"joodb_".$fieldname);
//                        $formfield = '<textarea class="inputbox '.$required.'" cols="60" rows="5"  name="'.$fieldname.'" id="'.$fid.'">'.stripslashes($value).'</textarea>';
                    ...
                    case 'longblob' :
                        ...
                        break;
                    default:
                        $formfield = '<input class="inputbox '.$required.'" type="text" name="'.$fieldname.'" id="'.$fid.'" value="'.htmlspecialchars(stripcslashes($value), ENT_COMPAT, 'UTF-8').'" maxlength="254" size="50" />';
                }
                return $formfield;
                break;
            }
