...
// 		vmdebug('checkFilterDir '.$filter_order_Dir);
	...
	public function TodosOsids($object, $select, $joinedTables, $whereString = '', $groupBy = '', $orderBy = '', $filter_order_Dir = '', $nbrReturnProducts = false){
	 ...
	 */
	 ...
	 //tiago hack
	public function exeSortSearchListQuery($object, $select, $joinedTables, $whereString = '', $groupBy = '', $orderBy = '', $filter_order_Dir = '', $nbrReturnProducts = false, $idsaexcluir = array()){

		// 		vmSetStartTime('exe');
		// 		if(USE_SQL_CALC_FOUND_ROWS){

		if( sizeof($idsaexcluir)>0 ) {
			$whereString = $whereString . ' AND p.virtuemart_product_id IN ' . "(".implode(',',$idsaexcluir).")" . ' ';
		}
...
class VmPagination extends JPagination {
	...
	function getLimitBox($sequence=0)
	{
		...
		if ($app->isAdmin()) {
			...
			$html = JHTML::_('select.genericlist',  $limits, '', 'class="inputbox btn" style="width:70px;" size="1" '.$js , 'value', 'text', $selected);
		}
		return $html;
	}
