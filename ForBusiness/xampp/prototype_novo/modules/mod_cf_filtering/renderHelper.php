...
class ModCfilteringRender{
			...
			if(count($options_ar)>0){
				foreach($disp_typeArray as $disp_type){
							...
							if($option_anchor){
								...
								if(($disp_type==2 || $disp_type==3) && empty($opt->isparent)){
									...
									if($disp_input){
										...
										$innerHTML.='<label class="'.$class_name.'" for="'.$element_id.'">'.$option_anchor.'</label>';
										if($dispCounter && isset($opt->counter) && $opt->active){
					...
					//--Range inputs--//
						...
						if(empty($options_ar[0]['maxlength']))$maxlength_from=7;
						else $maxlength_from=$options_ar[0]['maxlength'];
						if(empty($options_ar[0]['size']))$size_from=7;
						else $size_from=$options_ar[0]['size'];

						if(empty($options_ar[1]['maxlength']))$maxlength_to=7;
						else $maxlength_to=$options_ar[1]['maxlength'];
						if(empty($options_ar[1]['size']))$size_to=7;
						else $size_to=$options_ar[1]['size'];
						...
						$innerHTML.='<label for="'.$display_key.'_from"></label>'.$symbol_start;
						$innerHTML.='<input name="'.$key.'_from" value="'.$val_from.'" type="text" size="'.$size_from.'" maxlength="'.$maxlength_from.'" id="'.$display_key.'_from" '.$script.'>'.$symbol_end;

						$innerHTML.='<label for="'.$display_key.'_to">'.JText::_('MOD_CF_RANGE_TO').'</label>'.$symbol_start;
						$innerHTML.='<input name="'.$key.'_to" value="'.$val_to.'" type="text" size="'.$size_to.'" maxlength="'.$maxlength_to.'" id="'.$display_key.'_to" '.$script.'>'.$symbol_end;

						if(!$sliderExist && $this->results_trigger!='btn' && ($val_from || $val_to)){//generate the clear link
						...
						if($innerHTML){
							...
							$html.=$innerHTML;
							if(!$sliderExist){
								$html.='<input type="button" class="cf_range_btn cf_search_button" id="'.$display_key.'_button'.'"/>';
							...
							if(!$sliderExist)$html.=$innerHTML_clear;
							$html.='</div>';
						}
					}
					...
					//--Range Slider--//
						...
						if($inputTextExist){//an input text exist which stores the slider values
							...
							$innerHTML.='<input name="'.$key.'_from" value="'.$val_from.'" type="hidden" id="'.$display_key.'_from">';
						}
						...
						if($inputTextExist){//an input text exist which stores the slider values
							...
							$innerHTML.='<input  name="'.$key.'_to" value="'.$val_to.'" type="hidden" id="'.$display_key.'_to">';
						}
						...
						if($innerHTML){
							...
							if($this->results_trigger!='btn' && $this->results_loading_mode!='ajax'){
								$html.='<input type="button" id="'.$display_key.'_button'.'" class="cf_range_btn cf_search_button"/>';
	...
	private function getURL($filter,$var_value=NULL,$type='option'){
		...
		}else if($on_category_reset_others)$q_array['virtuemart_category_id']=$categ_array;//on category selection clear others
	...
	public function setSliderScripts($filter_key,$inputs=false,$slider_min_value=0,$slider_max_value=100){
		...
		$js_process="{$display_key}_sliderObj = new Cfslider('$display_key','$module_id', {
									...
									if(pos.min_moved){
										document.id('".$display_key."_from').value=pos.minpos;							
									...
									if(pos.max_moved){
										document.id('".$display_key."_to').value=pos.maxpos;
	...
	public function getScriptAssets(){
		$this->scriptFiles[]=JURI::base().'modules/mod_cf_filtering/assets/slider.js';
		$this->scriptFiles[]=JURI::base().'modules/mod_cf_filtering/assets/drag_refactor.js';
