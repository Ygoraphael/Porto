...
<body>
		...
		<div class="container-fluid-full">
			<div class="row-fluid">
				...
				<div id="content" class="span10">
					<?php 
						if(isset($_GET["mhstamp"])) {
							$dados_intervencao = get_intervencao(trim($_GET["mhstamp"]));
							if(!sizeof($dados_intervencao)) {
								return;
							}
						}
						else {
						...
						$dados_intervencao = $dados_intervencao[0];
						
						$current_page = "Intervenção nº " . $dados_intervencao["u_movid"];
					...
					<div class="row-fluid">
						<div class="span12">
							...
							<button type="submit" class="btn btn-primary" onclick="return false;" ><i class="white halflings-icon hdd"></i> Gravar</button>
							<form id="new_inter" role="form" data-toggle="validator" class="form-horizontal">
										...
										<select onchange="filtra_projetos()" id="cliente" class="span8 form-control" required>
											...
											<?php
												foreach($clientes as $cliente) {
													$selected = '';
													if($dados_intervencao["no"] == $cliente["no"])
														$selected = 'selected';
													echo "<option $selected value='" . $cliente["no"] . "'>" . $cliente["nome"] ."</option>";
												}
											?>
										</select>
									</div>
										...
										<select id="tarefa" class="span8 form-control" required>
											<?php
												foreach($tarefas as $tarefa) {
													$selected = '';
													if($dados_intervencao["mhtipo"] == $tarefa["campo"])
														$selected = 'selected';
													echo "<option $selected value='" . $tarefa["dytablestamp"] . "'>" . $tarefa["campo"] . "</option>";
												}
											?>
										</select>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" class="span8" value="<?php echo $dados_intervencao["pquem"]; ?>" id="quem_pediu">
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="quem_pediu">Inicio</label>
									<div class="controls">
										<input type="date" class="span2 auto_trigger" id="data_inicio" value="<?php echo substr($dados_intervencao["data"], 0, 10); ?>">
										<input type="text" class="span1 auto_trigger" placeholder="<?php echo date('H'); ?>" value="<?php echo substr($dados_intervencao["hora"], 0, 2); ?>" id="hora_inicio">
										<input type="text" class="span1 auto_trigger" placeholder="<?php echo date('i'); ?>" value="<?php echo substr($dados_intervencao["hora"], 3, 4); ?>" id="minuto_inicio">
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="quem_pediu">Fim</label>
									<div class="controls">
										<input type="date" class="span2 auto_trigger" id="data_fim" value="<?php echo substr($dados_intervencao["data"], 0, 10); ?>">
										<input type="text" class="span1 auto_trigger" placeholder="<?php echo date('H'); ?>" value="<?php echo substr($dados_intervencao["horaf"], 0, 2); ?>" id="hora_fim">
										<input type="text" class="span1 auto_trigger" placeholder="<?php echo date('i'); ?>" value="<?php echo substr($dados_intervencao["horaf"], 3, 4); ?>" id="minuto_fim">
									</div>
								</div>
								<div class="control-group">
								...
								<div class="control-group">
									...
									<div class="controls">
										<textarea class="form-control span8" id="relatorio" style="height:30vh; background:white;" required><?php echo $dados_intervencao["relatorio"]; ?></textarea>
									</div>
								</div>
	...
	<script>
		$('#new_inter').validator().on('submit', function (e) {
			if (e.isDefaultPrevented()) {
				...
				// ActivateLoading();
					...
					var strajx = "atualiza_intervencao('";
		...
		$(function() {
			...
			CalculaTempo();
			<?php 
				if($dados_intervencao["fref"]<>"") {
					echo "filtra_projetos();";
				}
			?>
		})	
	</script>
