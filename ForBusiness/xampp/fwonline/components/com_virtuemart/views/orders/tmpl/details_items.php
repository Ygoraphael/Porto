...
<table width="<?php echo $widthTable ?>%" cellspacing="0" cellpadding="0" border="0">
	<tr align="left" class="sectiontableheader">
		...
		<th align="left" colspan="2" width="<?php //echo $widthTitle ?>15%" ><?php echo JText::_('COM_VIRTUEMART_PRODUCT_NAME_TITLE') ?></th>
		<th align="center" width="15%"><?php echo JText::_('COM_VIRTUEMART_CART_COLECTION') ?></th>
		...
		<th align="right" width="10%" ><?php echo JText::_('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_TAX') ?></th>
		  <?php } ?>
		<th align="right" width="1%" style="padding-left:1%"><?php echo JText::_('COM_VIRTUEMART_ORDER_PRINT_TOTAL') ?></th>
	</tr>
<?php
	$db = JFactory::getDbo();
	...
	foreach($this->orderdetails['items'] as $item) {
		...
		$sql .= "		inner join e506s_virtuemart_product_categories B on A.virtuemart_product_id = B.virtuemart_product_id ";
		$sql .= "		inner join e506s_virtuemart_categories_".str_replace('-', '_', strtolower(strval($language->getTag())))." C on B.virtuemart_category_id = C.virtuemart_category_id ";
		$sql .= "	where A.virtuemart_product_id = " . $item->virtuemart_product_id ;
...
?>
		<tr valign="top">
			<td align="left">
				<?php
					$pieces = explode("|||", $item->order_item_sku);
					echo $pieces[0];
				?>
			</td>
			<td align="left" colspan="2" >
				<div><a href="<?php
					if( $lang->getTag() == 'en-GB' ) {
						$_link = str_replace("en/produtos", "en/products", $_link);
					}
					echo $_link;
					?>"><?php echo $item->order_item_name; ?></a></div>
				<?php
			...
			<td align="left">
				<?php echo $result[0]->category_name; ?>
			</td>
...
?>
 ...
 <tr class="sectiontableentry1" style="margin-top:10%">
			<td colspan="6" style="padding-right:10px;" align="right"><?php echo JText::_('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_PRICES_TOTAL'); ?></td>

                        <?php if ( VmConfig::get('show_tax')) { ?>
			<td  align="left"><?php echo "<span  class=''>".$this->currency->priceDisplay($this->orderdetails['details']['BT']->order_tax,$this->currency)."</span>" ?></td>
                        <?php } ?>
...
<hr>
	...
	<tr>
		<td align="right" class="pricePad" colspan="6"  style="padding-right:10px;"><?php echo JText::_('COM_VIRTUEMART_ORDER_PRINT_SHIPPING') ?></td>


			<?php if ( VmConfig::get('show_tax')) { ?>
				<td align="left"><?php echo "<span  class='priceColor2'>".$this->currency->priceDisplay($this->orderdetails['details']['BT']->order_shipment_tax, $this->currency)."</span>" ?></td>
                                <?php } ?>
...
<tr>
		<td align="right" class="pricePad" colspan="6"  style="padding-right:10px;"><?php echo JText::_('COM_VIRTUEMART_ORDER_PRINT_PAYMENT') ?></td>

			<?php if ( VmConfig::get('show_tax')) { ?>
				<td align="left"><?php echo "<span  class='priceColor2'>".$this->currency->priceDisplay($this->orderdetails['details']['BT']->order_payment_tax, $this->currency)."</span>" ?></td>
                                <?php } ?>
	...
	<tr>
		<td align="right" class="pricePad" colspan="6" style="padding-right:10px;"><strong><?php echo JText::_('COM_VIRTUEMART_ORDER_PRINT_TOTAL') ?></strong></td>
