 ...
 * v15: more alterations to fit Joomla 3.0
 * v16: added K2 class; small fix to VM class (is_product_special)
 * v17 VM: fixed detection of categories for child products
 * v18: added Adminswitches class
 *		fixes to detection of articlesave and com_finder search pages
 *		added detection of "wrapper" page type
 * v19 VM: fixed detection of category pagetypes after VM2.20b
 * v20 VM: shopper group now returns an array rather than single id
 * v21 Core: browser and browsertype now return an array of types, as type can be more than one (android, safari etc)
 */
...
if (!defined('JOMGENIUS_CLASS_LOADED')) {
	...
	class JomGeniusFactory extends JObject {
		...
		function factoryForName( $name ) {
			...
			$file = dirname(__FILE__) . '/jomgenius/' . $name . '.php';
			if ( !file_exists( $file ) ) {
				$ret = null;
				return $ret;
			}
	...
	class JomGeniusParent extends JObject {
		...
		function componentExists( $name ) {
			...
			$admin_component = JPATH_SITE . "/administrator/components/" . $name;
			$component = JPATH_SITE . "/components/" . $name;
		...
		static function UTC_strtotime( $date ) {
