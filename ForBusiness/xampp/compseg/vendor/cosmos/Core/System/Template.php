...
class Template {
    ...
    protected $valuesTranslate = array();
    ...
    private static $REG_NAME_TRANSLATE = "([[:alnum:]]|\s)+";
    ...
    public function existsTranslate($varname) {
        return in_array($varname, $this->varsTranslate);
    }
    ...
    private function identifyVars(&$content) {
        ...
        $r = preg_match_all("/%%" . self::$REG_NAME_TRANSLATE . "%%/", $content, $m);
        if ($r) {
            for ($i = 0; $i < $r; $i++) {
                if (!in_array($m[0][$i], $this->varsTranslate)) {
                    $this->varsTranslate[] = $m[0][$i];
                    $this->setTranslate(str_replace("%", "", $m[0][$i]), (new \App\Model\Translate())->translater(str_replace("%", "", $m[0][$i])));
                }
            }
        }
    }
    ...
    protected function setValue($varname, $value) {
        $translate = (new \App\Model\Translate());
        $r = preg_match_all("/%%" . self::$REG_NAME_TRANSLATE . "%%/", $value, $m);
        if ($r) {
            for ($i = 0; $i < $r; $i++) {
                $value = str_replace($m[0][$i], (new \App\Model\Translate())->translater(str_replace("%", "", $m[0][$i])), $value);
            }
        }
        $this->values['{{' . $varname . '}}'] = $value;
    }
    
    protected function setTranslate($varname, $value) {
        $this->valuesTranslate['%%' . $varname . '%%'] = $value;
    }
    ...
    private function getTranslate($varname) {
        return $this->valuesTranslate['%%' . $varname . '%%'];
    }
    ...
    public function clearTranslate($varname) {
        $this->setTranslate($varname, "");
    }
    ...
    protected function substTranslate($value) {
        // Common variables replacement
        $s = str_replace(array_keys($this->valuesTranslate), $this->valuesTranslate, $value);
        return $s;
    }
    ...
    public function parse() {
        ...
        $tmp = preg_replace("/{{(" . self::$REG_NAME . ")((\-\>(" . self::$REG_NAME . "))*)?((\|.*?)*)?}}/", "", $this->subst($this->getVar(".")));
        return preg_replace("/{{(" . self::$REG_NAME . ")((\-\>(" . self::$REG_NAME . "))*)?((\|.*?)*)?}}/", "", $this->substTranslate($tmp));
    }
