...
class GantryFeatureStyleDeclaration extends GantryFeature {
	...
	function init() {
        ...
        $css = 'a, ul.menu li .separator,.title, .component-content h2,h1,h2,h3,h4,h5,h6{color:'.$gantry->get('linkcolor').';}';
        $css .= '.button, .readon, .readmore, button.validate, #member-profile a, #member-registration a, .formelm-buttons button, .btn-primary {border-color:'.$linkColor->darken('20%').';vertical-align:inherit;}';
		
		//start special preset
		// Dynamic Preset Background
		$css .='div.rox-close,div.rox-carousel-hidden,h2.object_sbtitle1{background:'.$gantry->get('linkcolor').'!important;}';
		// Dynamic Preset Link Color
		$css .='.rox_obcontent .slide3 h1, .rox_obcontent .intro h1, .rox_obcontent .slide2 h1,.featured-view h4, .latest-view h4, .topten-view h4, .recent-view h4, .related-products-view h4,.featured-view .spacer span, .latest-view .spacer span, .topten-view .spacer span, .recent-view .spacer span, .related-products-view .spacer span, .browse-view .row .product .spacer span, span.PricesalesPrice,#rt-footer ul li:hover,div.catItemHeader h3.catItemTitle a,div.itemHeader h2.itemTitle,div.tagItemHeader h2.tagItemTitle a,div.userBlock h2,div.userItemHeader h3.userItemTitle a,.gf-menu .dropdown ul li:hover > .item,.login h1,div.ecwid-productBrowser-head,html#ecwid_html body#ecwid_body div.ecwid-productBrowser-price{color:'.$gantry->get('linkcolor').'!important;}';
        //preset adjust
		$css .='.gf-menu li:hover, .gf-menu li.active:hover,.gf-menu.l1 > li.active{border-top:5px solid '.$gantry->get('linkcolor').';}';
		$css .='.gf-menu .dropdown,.gf-menu .dropdown .grouped ol{border-left:3px solid '.$gantry->get('linkcolor').';}';
		$css .='div.open{border-bottom:3px solid '.$gantry->get('linkcolor').' !important;}';
		
		//Ecwid Adjustment
		$css .='html#ecwid_html body#ecwid_body div.ecwid-AddToBagButton-up, html#ecwid_html body#ecwid_body div.ecwid-AddToBagButton-up-hovering, html#ecwid_html body#ecwid_body div.ecwid-AddToBagButton-ie6-up, html#ecwid_html body#ecwid_body div.ecwid-AddToBagButton-ie6-up-hovering{background:'.$gantry->get('linkcolor').' url(http://themerox.com/themeforest/bestshop/templates/rox_bestshop/images/cartbgecwd.png) no-repeat 50% !important;}';
		
		$css .='html#ecwid_html body#ecwid_body div.ecwid-productBrowser-subcategories-categoryName span{background:'.$gantry->get('linkcolor').'!important;}';
        ...
        $css .= '.button:hover, .readon:hover, .readmore:hover, button.validate:hover, #member-profile a:hover, #member-registration a:hover, .formelm-buttons button:hover, .btn-primary:hover,div.rox_pdetails_tab .nav-tabs > .active > a, .nav-tabs > .active > a:hover,div.my-groovy-style a.continue, div.my-groovy-style a.showcart,a.vm-button-correct,div.front_pdetails_tab .nav-tabs > .active > a, .nav-tabs > .active > a:hover{background-color: '.$linkColor->lighten('10%').'; '.$this->_createGradient('top', $linkColor->lighten('10%'), '1', '0%', $linkColor->darken('3%'), '1', '100%').'}'."\n";
