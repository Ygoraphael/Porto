...
if ( JFactory::getApplication()->isSite() ) {
	...
	if (!in_array('JModuleHelper', $classes) && !in_array('jmodulehelper', $classes)) {
		if ( version_compare( JVERSION, '3.0', '<=' ) ) { // J1.7, 2.5
			require_once dirname(__FILE__).'/overloaded/modulehelper.php';
		} else {
			require_once dirname(__FILE__).'/overloaded/modulehelper30.php';
		}
	}
...
function plgSystemMetamodProRenderModule( &$module, &$attribs) {
//	print_r($module);
//	print_r($attribs);
	
	$registry = new JRegistry( $module->params );
	$style = $registry->get('mm_force_style');
	if ($style != '0' and $style != '') {
		$template = JFactory::getApplication()->getTemplate();
		$attribs['style'] = preg_replace('/^(system|' . $template . ')\-/i', '', $style);
	}
...
class plgSystemMetamodPro extends JPlugin
{
	...
	function onAfterInitialise()
	{
		...
		$app->registerEvent('onRenderModule', 'plgSystemMetamodProRenderModule');		
	}
	...
	*/
	...
	public static function expandMetaMod( $metamod ) {
		...
		// retrieve the style2 parameter from the MetaMod.
		$registry = new JRegistry( $metamod->params );
		if ( version_compare( JVERSION, '3.0', '<' ) ) { // J1.7, 2.5
			$style2 = $registry->get( 'force_style', $registry->get( 'style' ) );
		} else {
			$style2 = $registry->get( 'force_style', $registry->get( 'style2' ) );
		}
		if ( $content != '' ) {
			...
			// merge in the forced style, if there was one, for the new "content" module:
			if ($style2) {
				$registry->merge( new JRegistry( array('mm_force_style'=>$style2) ) );
				$metamod->params = $registry->toString();
			}
		...
		$quoted_mod_identifiers = "'". implode( "','", array_map( array( $db, 'escape'), $ids ) ) . "'"; //J3.0 changed getEscaped to escape
		...
		for($i = 0; $i < $total; $i++) {
			...
			if ($style2) {
				$registry = new JRegistry( $modules[$i]->params ); // get the current params
				$registry->merge( new JRegistry( array('mm_force_style'=>$style2) ) );
				$modules[$i]->params = $registry->toString();
			}
	...
	public static function metaModVersion( $which = 'both') {
		static $major = '3';
		static $minor = '13b';
		static $all = '3.13b';
		
		if ( !is_callable( array( 'modMetaModHelper', 'version') ) ) {
			$path = JPATH_BASE.'/modules/mod_metamod/helper.php';
			if ( file_exists( $path ) ) {
				include_once($path);
			}
		}
	...
	public static function metaModVersionCompare( $version, $comp ) {
	...
	public static function metaModIncludes( $module )
	{
		static $chrome;
	...
	public static function &changeCache( $function = 'get', $changeObject = null ) {
		static $cache = null;
		if ($cache == null) {
			$cache = new stdclass();
			$cache->modules = array();
	...
	public static function applyParamChanges( &$modules ) {
