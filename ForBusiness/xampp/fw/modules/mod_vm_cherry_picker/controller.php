...
class chpController {
    ...
    public function getFilters($offset = null) {
        ...
        foreach ($filters as $i => $filter) {
            if (chpconf::option('useseemore') && chpconf::option('use_seemore_ajax') && $filters_shown == chpconf::option('b4seemore') && !$mode) {
                $s .= chpWriter::appendSeeMore();
                break;
            }
            ...
            if (chpconf::option('fill_metatitle') && $filter_selected && !$offset)
                $this->addToPageTitle($filter);
            if (!$param_has_filter && $filter_selected)
                $param_has_filter = true;
        }

        // add Clear link at top
        if (chpconf::option('mode') == 1 && chpconf::option('showclearlink') && $param_has_filter && !$offset)
            $s = $this->getClearLink() . $s;

        // hide parameters with 1 filter
        if (chpconf::option('hide_params_with1filter') && $param_has_filter == false && $filters_shown < 2 && !$offset && !(chpconf::option('useseemore') && chpconf::option('use_seemore_ajax')))
            return false;
    ...
    public function getFilterCount($filter) {
            ...
            foreach ($this->parameters as $i => $p) {
                if ($i == $this->curr_param_index) {
                    ...
                    if (chpconf::option('short_url') || $mode == 1 || $mode == 2) {
                        ...
                        foreach ($this->applied_filters[$i] as $n) {
                            $f .= $p['parameter_name'] . '_' . $n . ',';
                        }
                    }
                }
            }
        ...
        if ($this->usecache) {
            $this->writeToCache($needle, $count);
        }
            ...
            if (chpconf::option('showfiltercount') != NUM_PROD_NOT_CALC && chpconf::option('usesmartsearch')) {
                ...
                if ($this->parameters_applied > 1 && $filter_selected) {
                    ...
                    if (count($a) > 1) {
                            ...
                            if ($i != $curr_index) {
                                ...
                                if ($p['parameter_type'] != 'V') {
                                    ...
                                    foreach ($result as $value) {
                                        if ($value) {
                                            array_push($w, "FIND_IN_SET(\"$value\",REPLACE(`{$p['parameter_name']}`,';',','))");
                                        }
                                    }
                                    if ($w)
                                        $where .= " AND (" . join(" OR ", $w) . ")";
                        ...
                        $count = $db->loadResult($query);
                        if ($count == 0)
                            return $this->getClearParameterUrl();
                    }
                }
            }
            //	if(!$url){
                ...
                for ($i = 0; $i < $j; $i++) {
                    if ($curr_index == $i) {
                        ...
                        if ($filter_selected) {
                            ...
                            if ($result)
                                $url .= '&' . $this->filter_info[$i]['param_name'] . '=' . urlencode(implode('|', $result));
    ...
    public function apprehendBaseQuery() {
        ...
        if ($lp && !$hp) {
            $where .= " AND `product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100))>=" . $lp;
        } elseif (!$lp && $hp) {
            $where .= " AND `product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100))<=" . $hp;
        } elseif ($lp && $hp && ($hp > $lp)) {
            $where .= " AND `product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100)) BETWEEN " . $lp . " AND " . $hp;
        }
        ...
        foreach ($this->parameters as $i => $p) {
            ...
            if ($p['parameter_type'] != "V") {
                if (chpconf::option('mode') == 0) { // if single mode
                    ...
                    $param_name = $param;
                    if ($get)
                        $this->where_clause[$i] = ' AND pt.`' . $p['parameter_name'] . '`="' . $get[0] . '"';
                }else {
                    $comp_value = 'in';
                    $param_name = $param . '[]';
                    //if($get) $this->where_clause[$i]=" AND `{$p['parameter_name']}` IN (\"".( is_array($get) ? join("\",\"",$get) : $get)."\")";
                    if ($get)
                        $this->where_clause[$i] = " AND `{$p['parameter_name']}` IN (\"" . join("\",\"", $get) . "\")";
                }
            }else {
                if (chpconf::option('mode') == 0) {
                    ...
                    $param_name = $param;
                    if ($get)
                        $this->where_clause[$i] = " AND FIND_IN_SET(\"{$get[0]}\",REPLACE(pt.`{$p['parameter_name']}`,';',','))";
                }else {
                    ...
                    if ($get) {
                        ...
                        foreach ($get as $value) {
                            if ($value) {
                                array_push($w, "FIND_IN_SET(\"$value\",REPLACE(pt.`{$p['parameter_name']}`,';',','))");
                            }
                        }
                        if ($w)
                            $this->where_clause[$i] = " AND (" . join(" OR ", $w) . ")";
                    }
                }
            }
            if (chpconf::option('short_url')) {
    ...
    public function getVMOrderByForm() {
        ...
        foreach ($parameters as $p) {
            ...
            if (!$get) {
                ...
                if (!$get)
                    continue; // nothing in url, continue to next parameter
            }
            if (is_array($get)) {
                foreach ($get as $g) {
            ...
            if ($comp)
                echo '<input type="hidden" name="product_type_' . $ptid . '_' . $p['parameter_name'] . '_comp" value="' . $comp . '" />';
        }
        ...
        if ($url_ptid)
            echo '<input type="hidden" name="ptid" value="' . $url_ptid . '">';
    }
    ...
    public function getVMPriceQuery($tax = false) {
        ...
        if ($this->low_price && !$this->high_price) {
            return "`product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100))>=" . $this->low_price;
        } elseif (!$this->low_price && $this->high_price) {
            return "`product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100))<=" . $this->high_price;
        } elseif ($this->low_price && $this->high_price && ($this->high_price > $this->low_price)) {
            return "`product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100)) BETWEEN " . $this->low_price . " AND " . $this->high_price;
        }
    ...
    public function getBackLink() {
        ...
        if (chpconf::option('fill_metatitle'))
            $this->addToPageTitle(implode(', ', $this->applied_filters[$this->curr_param_index]));
    ...
    public function getTrackbarParameter() {
        ...
        if ($url_value) {
            if ($mode == 1) {
                ...
                $v = explode(':', $url_value);
                if ($v[0]) {
                    $key = array_search($v[0], $poss_values);
                    if ($key !== false)
                        $vl = $key;
                }
                if ($v[1]) {
                    $key = array_search($v[1], $poss_values);
                    if ($key !== false)
                        $vr = $key;
                }
                //print_r($v);
            ...
            chpWriter::setParamHasAppliedFilter(true);
            if (chpconf::option('fill_metatitle'))
                $this->addToPageTitle($plabel . ': ' . $url_value);
        }else {
        ...
        if (chpconf::option('tb_lbl_type') == 0) {
            ...
            $s .= '<span class="chp-pcl" data-pname="' . $pname . '" onclick="clearRefine(this,chpform' . chpconf::option('module_id') . ')" onmouseover="clearHover(this,true)" onmouseout="clearHover(this,false)">' . chpconf::option('clear') . '</span>';
        $s .= '<div class="chp-tbouter"><div id="chptb-' . $pname . '" class="chp-trackbar hid"><div class="chp-fullrange"></div>';
        if ($mode == 2)
            $s .= '<div class="chp-selrange"></div>';
        $s .= '<div class="left-slider"></div>';
        if ($mode == 2)
            $s .= '<div class="right-slider"></div>';
...
//inputRight:null,
    ...
    public function setBaseUrl() {
        ...
        if (chpconf::option('append_ptid') || chpconf::option('custom_ptid'))
            $this->baseurl .= '&ptid=' . $this->ptid();

        if ($mid)
            $this->baseurl .= '&manufacturer_id=' . $mid;
        if ($this->low_price || $this->high_price) {
            $this->baseurl .= '&low-price=' . $this->low_price . '&high-price=' . $this->high_price;
        }
        if ($debti)
            $this->baseurl .= '&chp=showtime';
    ...
    private function assembleAppliedFiltersUrl() {
        ...
        if (chpconf::option('append_ptid'))
            $url = 'ptid=' . $this->ptid() . $url;
    ...
    public function setPageTitle() {
        if ($this->low_price || $this->high_price) {
            ...
            if ($this->low_price && !$this->high_price) {
                $this->pagetitle .= " (\${$this->low_price} & Above)";
            }
            // we have only high-price
            else if (!$this->low_price && $this->high_price) {
                $this->pagetitle .= " (\${$this->high_price} & Under)";
            }
            // else, both
            else {
                $this->pagetitle .= " (\${$this->low_price} - \${$this->high_price})";
            }
        }
        if ($this->pagetitle) {
...
<input type="hidden" name="limitstart" value="0"/>';
        ...
        $j = count($this->parameters);
        for ($i = 0; $i < $j; ++$i) {
            if (isset($this->parameters[$i]['mode']) && ($this->parameters[$i]['mode'] == 1 || $this->parameters[$i]['mode'] == 2))
                continue; // skip trackbar parameters
            if ($this->applied_filters[$i]) {
                if (chpconf::option('short_url')) {
    ...
    public function getSearchByPrice() {
        ...
        if (($this->low_price && !$this->high_price) || (!$this->low_price && $this->high_price) || ($this->low_price && $this->high_price && ($this->high_price > $this->low_price))) {
            ...
            if ($manufacturer_id) {
                $baseurl .= '&manufacturer_id=' . $manufacturer_id;
            }
        ...
        if (chpconf::option('showtrackbar'))
            $priceform = $this->getPriceTrackbar() . $priceform;
...
<div class="right-slider"></div>
    ...
    private function getExtremumPrice($category_id, $ptid) {
        $db = & JFactory::getDBO();
        if (!empty($category_id)) {
            $q = "SELECT MIN(`product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100))) AS leftLimit, MAX(`product_price`*(1+((select calc_value from e506s_virtuemart_calcs where virtuemart_calc_id = 1 limit 1)/100))) AS rightLimit" .
    ...
    public function checkCache() {
        $manufacturer_id = JRequest::getVar('manufacturer_id', '');
        if ($this->low_price || $this->high_price || $manufacturer_id)
            return;
    ...
    private function getFromCache($needle) {
        if (empty($this->cache)) {
            ...
            if ($pos === false) {
                ...
                $v = substr($this->cache, $pos + strlen($needle) + 1, 6);
                return $v += 0;
            }
        }
    }
