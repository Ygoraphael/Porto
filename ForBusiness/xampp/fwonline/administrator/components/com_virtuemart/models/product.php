...
// 		vmdebug('$product->field',$product->field);
	...
	public function getStock ($product) {
		...
		$query->select("product_in_stock - product_ordered product_in_stock");
		$query->from($db->quoteName('#__virtuemart_products'));
		$query->where($db->quoteName('virtuemart_product_id') . ' = '. $product);
		$db->setQuery($query);
		$results = $db->loadObject();
		
		return $results->product_in_stock;
