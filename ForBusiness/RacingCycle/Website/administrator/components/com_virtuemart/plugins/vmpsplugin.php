	...
	*/
	...
	protected function getPluginHtml ($plugin, $selectedPlugin, $pluginSalesPrice) {
		...
		if ($pluginSalesPrice) {
			...
			$costDisplay = '<span class="' . $this->_type . '_cost"> (' . JText::_ ('COM_VIRTUEMART_PLUGIN_DISC_DISPLAY') . $costDisplay . ")</span>";
		}
	...
	function getCosts (VirtueMartCart $cart, $method, $cart_prices) {
		...
		//hack tiago
		if( $method->virtuemart_paymentmethod_id == 1 ) {
			...
			if( sizeof($result) > 0 ) {
				$method->cost_percent_total = -($result[0]->discount_pp);
			}
		}
	...
	function setCartPrices (VirtueMartCart $cart, &$cart_prices, $method, $progressive = true) {
		...
		if($this->_psType=='payment'){
			...
			//	$cartTotalAmount = ($cartTotalAmountOrig + $method->cost_per_transaction) * (1 +($method->cost_percent_total * 0.01));
				//$cartTotalAmount = $cart_prices['basePrice'] * (1 +($method->cost_percent_total * 0.01));
			//} else {
				//progressive
				//$cartTotalAmount = ($cartTotalAmountOrig + $method->cost_per_transaction) / (1 -($method->cost_percent_total * 0.01));
			//	$cartTotalAmount = $cartTotalAmountOrig - ($cartTotalAmountOrig - ($cartTotalAmountOrig * (1 +($method->cost_percent_total * 0.01))));
			//}
			$cartTotalAmount = $cartTotalAmountOrig - ($cartTotalAmountOrig - ($cartTotalAmountOrig * (1 +($method->cost_percent_total * 0.01))));
