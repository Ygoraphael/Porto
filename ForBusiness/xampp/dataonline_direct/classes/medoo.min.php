...
class medoo
	...
	protected
	function column_quote($string)
		{
		return '"' . str_replace('.', '"."', preg_replace('/(^#|\(JSON\))/', '', $string)) . '"';
		}

	protected
	function column_push($columns)
		{
		if ($columns == '*')
			{
			return $columns;
	...
	protected
	function inner_conjunct($data, $conjunctor, $outer_conjunctor)
		{
		$haystack = array();
	...
	protected
	function fn_quote($column, $string)
		{
		return (strpos($column, '#') === 0 && preg_match('/^[A-Z0-9\_]*\([^)]*\)$/', $string)) ? $string : $this->quote($string);
		}

	protected
	function data_implode($data, $conjunctor, $outer_conjunctor = null)
		{
		$wheres = array();
			...
			if (isset($where['LIKE']))
				...
				if (is_array($like_query))
					...
					foreach($like_query as $column => $keyword)
						{
						$keyword = is_array($keyword) ? $keyword : array(
							$keyword
						);
	...
	protected
	function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
		{
		$table = '"' . $table . '"';
	...
	function replace($table, $columns, $search = null, $replace = null, $where = null)
		{
		if (is_array($columns))
			{
			$replace_query = array();
	...
	function get($table, $columns, $where = null)
		{
		if (!isset($where))
			{
			$where = array();
	...
	function has($table, $join, $where = null)
		{
		$column = null;
	...
	function count($table, $join = null, $column = null, $where = null)
		{
	...
	function max($table, $join, $column = null, $where = null)
		{
	...
	function min($table, $join, $column = null, $where = null)
		{
	...
	function avg($table, $join, $column = null, $where = null)
		{
	...
	function sum($table, $join, $column = null, $where = null)
		{
