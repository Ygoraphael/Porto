...
public class Artigo extends Db {
   ...
   {    
        this.lastSQL = ""
            + "select stock.ref, stock.design, stock.cor, stock.tam, stock.usr2 modalidade, stock.usr5 genero, stock.usr3 linha, stock.familia, stock.u_codproj, stock.u_descproj, stock.epv2, stock.stock - isnull(cativo.stock, 0) stock, "
            + "isnull((select top 1 img1 from u_stproj where ststamp = stock.ststamp and u_codproj = stock.u_codproj), 'X:\\logotipo.png') img1, "
            + "isnull((select top 1 img2 from u_stproj where ststamp = stock.ststamp and u_codproj = stock.u_codproj), '') img2, "
            + "isnull((select top 1 img3 from u_stproj where ststamp = stock.ststamp and u_codproj = stock.u_codproj), '') img3, "
            + "isnull((select top 1 img4 from u_stproj where ststamp = stock.ststamp and u_codproj = stock.u_codproj), '') img4, "
            + "stock.categoria, "
            + "stock.descricao_pt, "
            + "(case when len(stock.descricao_uk) > 0 then stock.descricao_uk else stock.descricao_pt end) descricao_uk, "
            + "(case when len(stock.design_uk) > 0 then stock.design_uk else stock.categoria end) design_uk, "
            + "stock.desconto, "
            + "stock.novidade, "
            + "stock.destaque, "
            + "stock.antibact, "
            ...
            + "where stock.stock - isnull(cativo.stock, 0) > 0 and isnull((select top 1 pub from u_stproj where ststamp = stock.ststamp and u_codproj = stock.u_codproj), 0) = 1";
