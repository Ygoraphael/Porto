...
$data->cart_show = '<a class="linkColor" href="'.JRoute::_("index.php?option=com_virtuemart&view=cart".$taskRoute,$useXHTML,$useSSL).'" ><span>
<i id="shopCart" class="awesome-color fa fa-2x fa-shopping-cart" aria-hidden="true"></i></span></a>
<label for="shopCart" class="badge cart-badge badge-warning">'.$data->totalProduct.'</label>';
} else {
	echo '<a class="linkColor" href="'.JURI::base().'" ><span><i id="shopCart" class="awesome-color fa fa-2x fa-shopping-cart"></i></span></a>
	<label for="shopCart" class="badge cart-badge badge-warning">0</label>';
}


/* $data->cart_show = '<a href="<?php echo $this->baseurl ?>/index.php"><span><i class="fa fa-shopping-cart fa-lg" aria-hidden="true"></i></span></a>' */
/* $data->cart_show = '<a style ="float:right;" href="'.JRoute::_("index.php?option=com_virtuemart&view=cart".$taskRoute,$useXHTML,$useSSL).'">'.$data->totalProduct.'</a>'; */
