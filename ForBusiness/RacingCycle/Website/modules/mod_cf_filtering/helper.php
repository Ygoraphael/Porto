...
class ModCfFilteringHelper{
	...
	public function getFilters($params, $module){
		...
		if(empty($filters_order) || !in_array('virtuemart_category_id', $filters_order) || count($filters_order)!=count($this->fltSuffix))$filters_order=array('virtuemart_category_id','virtuemart_manufacturer_id','price','custom_f');
				...
				case 'price':
					if($this->optHelper->getDisplayControl('price_flt',$this->selected_flt,$params)){
						...
						if($display_price_inputs || $display_price_slider){
							...
							$price_flt['options'][0]['slider_min_value']=$params->get('price_flt_slider_min_value','0');
							//to
							$price_flt['options'][1]['size']=$cf_price_size;
							$price_flt['options'][1]['maxlength']=$cf_price_maxlength;
							$price_flt['options'][1]['slider_max_value']=$params->get('price_flt_slider_max_value','300');
