	...
	<script id="newfile-template-<?php echo $this->getId(); ?>" type="text/x-handlebars-template">
		<li id="{{file.id}}" class="pending">
			...
			<div class="file-image">
				<img src="" alt="<?php echo JText::_('COM_JUFORM_VIDEO_THUMBNAIL'); ?>"/>

				<div class="image-action">
					<span class="image-browser icon-new" title="<?php echo JText::_('COM_JUFORM_UPLOAD'); ?>"></span>
					<span class="image-remove icon-trash" data-icon-undo="icon-undo" data-icon-trash="icon-trash"
					      title="<?php echo JText::_('COM_JUFORM_REMOVE'); ?>"></span>
				</div>
			</div>
			<div class="file-data">
				...
				<div class="file-info-wrap">
					<div class="file-info file-size-wrap"><label
							class="info-label"><?php echo JText::_('COM_JUFORM_SIZE'); ?>:</label><span
							class="info-value">{{file.sizeFormatted}}</span></div>
					<div class="file-info file-type-wrap"><label
							class="info-label"><?php echo JText::_('COM_JUFORM_TYPE'); ?>:</label><span
							class="info-value">{{file.type}}</span></div>
				...
				<input type="hidden" class="file-target" value=""
				       name="<?php echo $this->getName(); ?>[{{key}}][target]"/>
				<input type="hidden" class="file-image-newname" value=""
				       name="<?php echo $this->getName(); ?>[{{key}}][imagenewname]"/>
				<input type="hidden" class="file-image-target" value=""
				       name="<?php echo $this->getName(); ?>[{{key}}][imagetarget]"/>
				<input type="hidden" class="file-image-remove" value="0"
				       name="<?php echo $this->getName(); ?>[{{key}}][imageremove]"/>
				<input type="hidden" class="file-published" value="1"
				       name="<?php echo $this->getName(); ?>[{{key}}][published]"/>
				<input type="hidden" class="file-remove" value="0"
				       name="<?php echo $this->getName(); ?>[{{key}}][remove]"/>
				<input type="hidden" class="file-isnew" value="1"
				       name="<?php echo $this->getName(); ?>[{{key}}][is_new]"/>
		</li>
	</script>
<?php
...
if ($files)
{
	$imageUrl = JUri::root(true) . "/media/com_juform/field_attachments/media/" . $this->id . "_" . $this->submission_id . "/";
	...
	foreach ($files AS $key => $file)
	{
		...
		$iconImageRemove = 'icon-trash';
		if (isset($file['imageremove']) && $file['imageremove'] == 1)
		{
			$iconImageRemove = 'icon-undo';
			$class[]         = 'imageremove';
		}
		...
		{
			$html .= '<img src="' . $imageTmp . $file['imagetarget'] . '" alt="' . JText::_('COM_JUFORM_VIDEO_THUMBNAIL') . '" />';
		}
		else
		{
			$imageSrc = $file['imagename'] ? $imageUrl . $file['imagename'] : '';
			$html .= '<img src="' . $imageSrc . '" alt="' . JText::_('COM_JUFORM_VIDEO_THUMBNAIL') . '" />';
		}
		$html .= '<div class="image-action">';
		$html .= '<span class="image-browser icon-new" title="' . JText::_('COM_JUFORM_UPLOAD') . '"></span>';
		$html .= '<span class="image-remove ' . $iconImageRemove . '" data-icon-undo="icon-undo" data-icon-trash="icon-trash" title="' . JText::_('COM_JUFORM_REMOVE') . '"></span>';
		$html .= '</div>';
		$html .= '</div>';
...
$html .= "<span class=\"pickfiles btn btn-default btn-sm\"><i class=\"fa fa-plus-circle\"></i> " . JText::_('COM_JUFORM_SELECT_FILE') . "</span>";
$html .= " <span class=\"uploadfiles btn btn-default btn-sm disabled\"><i class=\"fa fa-upload\"></i> " . JText::_('COM_JUFORM_UPLOAD') . "</span>";
$html .= "<span class=\"pickimagefiles btn btn-default btn-sm hide\"><i class=\"fa fa-plus-circle\"></i> " . JText::_('COM_JUFORM_SELECT_FILE') . "</span>";
$html .= "</div>";
