	...
	<script id="newfile-template-<?php echo $this->getId(); ?>" type="text/x-handlebars-template">
		<li id="{{file.id}}" class="pending">
			...
			<div class="file-title-name">
				<input type="text" class="file-name" name="<?php echo $this->getName() ?>[{{key}}][name]"
				       value="{{file.name}}" placeholder="<?php echo JText::_('COM_JUFORM_FILE_NAME'); ?>"/>
				<input type="text" class="file-title" name="<?php echo $this->getName() ?>[{{key}}][title]" value=""
				       placeholder="<?php echo JText::_('COM_JUFORM_TITLE'); ?>"/>
			</div>
			<div class="file-info-wrap">
				...
				<!--<div class="file-info file-counter-wrap"><label
						class="info-label"><?php /*echo JText::_('COM_JUFORM_DOWNLOADS'); */?>:</label>
					<span class="info-value"><input type="text"
					                                name="<?php /*echo $this->getName() */?>[{{key}}][downloads]" value="0"/></span>
				</div>-->
...
if ($files)
{
	foreach ($files AS $key => $file)
	{
		...
		$html .= '<input type="text" class="file-name" name="' . $this->getName() . '[' . $key . '][name]" value="' . $file['name'] . '" placeholder="' . JText::_('COM_JUFORM_FILE_NAME') . '" />';
		...
		$html .= '<div class="file-info file-counter-wrap"><label class="info-label">' . JText::_('COM_JUFORM_DOWNLOADS') . ':</label>';
...
if(!$this->params->get('auto_upload', 0)){
    $html .= " <span class=\"uploadfiles btn btn-default btn-mini disabled\"><i class=\"icon-upload\"></i> " . JText::_('COM_JUFORM_UPLOAD') . "</span>";
}
$html .= "</div>";
