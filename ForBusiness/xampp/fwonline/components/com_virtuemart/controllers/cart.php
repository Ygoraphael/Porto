...
class VirtueMartControllerCart extends JController {
    ...
    public function display($cachable = false, $urlparams = false) {
        ...
        $var_STsameAsBT = JRequest::getInt('STsameAsBT', 2);
        if ($var_STsameAsBT != 2) {
            $cart->STsameAsBT = $var_STsameAsBT;
    ...
    public function addJS() {
        ...
        if ($cart) {
            ...
            if ($products) {
                ...
                if(count($customPlugins)) {
                    foreach( $customPlugins as $customValue ) {
                        $tamanho = $customValue["stockable"]["selectoptions1"];
                        $cor = $customValue["stockable"]["selectoptions2"];
                    }
                }
                else {
                    $tamanho = "";
                ...
                //error_log(print_r($products[0], true));
                
                $view->assignRef('tamanho', $tamanho);
                $view->assignRef('cor', $cor);
                ...
                $productData = $productModel->getProduct($virtuemart_product_ids[0]);

                $tmp = false;
                $view->assignRef('product', $tmp);
                $view->assignRef('productData', $productData);
            }
    ...
    public function update() {
        ...
        if ($cartModel->updateProductCart() == -1)
            $mainframe->enqueueMessage(JText::_('COM_VIRTUEMART_CART_PRODUCT_OUT_OF_QUANTITY'), 'error');
        else if ($cartModel->updateProductCart())
    ...
    public function VerificaStock() {
        $mainframe = JFactory::getApplication();
        $cart = VirtueMartCart::getCart();
        $db = JFactory::getDbo();

        foreach ($cart->products as $linha) {
            if (sizeof($linha->param) > 0) {
                $arrayKeys = array_keys($linha->param);
                $param = $linha->param[$arrayKeys[0]];

                $arrayKeys = array_keys($param);
                $param = $param[$arrayKeys[0]];

                $query = $db->getQuery(true);
                $query->select('product_in_stock - product_ordered real_stock');
                $query->from($db->quoteName('e506s_virtuemart_products'));
                $query->where($db->quoteName('virtuemart_product_id') . " = " . $db->quote($param["child_id"]));

                $db->setQuery($query);
                $row = $db->loadRow();

                if (sizeof($row) > 0) {
                    if ($row[0] < $linha->quantity) {
                        $sku = explode("|||", $linha->product_sku);
                        $sku = $sku[0];

                        JError::raiseWarning(100, JText::_('COM_VIRTUEMART_CART_PRODUCT_STOCK_CART_OUT_NOTIFY') . ' ' . $sku . ' - ' . $linha->product_name . ' - ' . $param["selectoptions1"] . " " . $param["selectoptions2"] . ".<br>" . JText::_('COM_VIRTUEMART_CART_PRODUCT_STOCK_CART_OUT_NOTIFY2'));
