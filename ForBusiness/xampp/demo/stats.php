	...
	<ul class="unstyled">
		...
		<li><span class="sparkLineStats7"></span>
			...
			<?php
				$query = "
					select 
						sum(case when year(ft.fdata)=".(date("Y")-1)." then ft.eivain1+ft.eivain2+ft.eivain3+ft.eivain4+ft.eivain5+ft.eivain6 else 0 end) as total 
					from ft (NOLOCK) 
		...
		<li><span class="sparkLineStats8"></span>
			...
			<?php
				$query = "
					select 
						sum(case when year(ft.fdata)=".date("Y")." then ft.eivain1+ft.eivain2+ft.eivain3+ft.eivain4+ft.eivain5+ft.eivain6 else 0 end) as total 
					from ft (NOLOCK) 
						inner join ft2 on ft.ftstamp = ft2.ft2stamp 
					where 	
						(ft.tipodoc=1 or ft.tipodoc=3) and 
