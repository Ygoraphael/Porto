...
class Admin extends CI_Controller {
	...
	public function ajax() {
		...
		if( isset($_SESSION["backoffice_user_id"]) ) {
			...
			switch( $sefurl ) {
				case 'delete_pages':
					$data['id'] = $this->input->post('stamp');
					$this->mssql->utf8_encode_deep( $data );
					$this->admin_model->delete_pages( $data );
					break;
	...
	public function translations()
	{
		...
		if( isset($_SESSION["user_id"]) && $_SESSION["user_id"] > 0 ){
			...
			if( $data['user']["is_admin"] == '1' ) {
				...
				$data['pagina'] = "";
				$data["translations"] = $this->translation_model->get_translations();
				$data['buttons'] = '2';
		...
		if( isset($_SESSION["user_id"]) && $_SESSION["user_id"] > 0 ){
			...
			if( $data['user']["is_admin"] == '1' ) {
				...
				if($id_post == ""){
					...
					$data['pagina'] = "-".$data["translation"]->keyvalue;
					$data['buttons'] = '1';
	...
	public function create_plugin()
	{
		...
		$result = $this->admin_model->create_plugin($url,$title,$position);
	...
	{ 
		...
		$result = $this->admin_model->create_menu_category($title,$url,$name,$logo,$position,$toggle);
	...
	{ 
		...
		$result = $this->admin_model->update_menu_category($id,$title,$url,$name,$logo,$position,$toggle);
	...
	public function update_translation()
	{
		...
		$result = $this->translation_model->update_translation($id,$key,$value,$language);
	...
	public function update_plugin()
	{
		...
		$result = $this->admin_model->update_plugin($id,$url,$title,$position);
		echo $result;
	}
