...
// 			$product -> product_price_edate = $tmpProduct -> product_price_edate;
            ...
            if (!empty($tmpProduct->customfieldsCart))
                $product->customfieldsCart = true; //why do we have this here?
            ...
            if ($product) {
                ...
                if (!empty($product->customPlugin)) {
                    $pieces = explode(":", $product->customPlugin);
                    $pieces = explode('"', $pieces[5]);
                    $productKey .= $pieces[1];
    ...
    public function updateProductCart($cart_virtuemart_product_id = 0, $quantity = null) {
        ...
        if (empty($cart_virtuemart_product_id))
            $cart_virtuemart_product_id = vRequest::getString('cart_virtuemart_product_id');
        if ($quantity === null)
    ...
    private function checkForQuantities($product, &$quantity = 0, &$errorMsg = '') {
        ...
        if ($quantity == 0) {
            $errorMsg = JText::sprintf('COM_VIRTUEMART_CART_ERROR_NO_VALID_QUANTITY', false);
            $this->setError($errorMsg);
            vmInfo($errorMsg . ' ' . $product->product_name);
            return false;
        }
        ...
        if (($stockhandle != 'none' && $stockhandle != 'risetime') || true) {
            //end tiago loureiro
