...
<?php foreach ($cart as $item): ?>
                                <li class="row">
                                    ...
                                    <span class="itemName" style="font-size:15px;"><?= substr($item['design'], 0, 100); ?></span>
                                    <span class="remove"><button type="button" onclick="window.location.href = '<?= base_url(); ?>ponto/delete_tmp_tarefa_stamp/<?= $item['ststamp']; ?>'"class="btn btn-danger" style="font-size:18px;color:white"><i class="fa fa-times" aria-hidden="true"></i></button></span>
                                </li>
<?php endforeach; ?>
                        </ul>
                    </div>
                    <div class="col-lg-12" style="margin-top:25px;" >
                        ...
                        <button id="conf_btn" onclick="window.location.href = '<?= base_url(); ?>ponto/save_tarefa'" class="btn_cart fancybox fancybox.iframe" data-toggle="modal" data-target="#popup" onclick="parent.jQuery.fancybox.close();"><i class="fa fa-check-square-o" style="font-size:46px;color:white"></i></button>
...
<script>
    ...
    $("#conf_btn").click(function () {
        ...
        insertBlocker("#page-wrapper", 30, 30, 34, 35);
