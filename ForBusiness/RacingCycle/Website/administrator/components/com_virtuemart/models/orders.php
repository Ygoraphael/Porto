...
// 			}
	...
	private function _createOrder($_cart, $_usr, $_prices)
	{
		...
		//tiago hack
		if( $_cart->virtuemart_paymentmethod_id == 1 ) {
			$db = JFactory::getDbo();
			$sql = "
				select discount_pp 
				from #__virtuemart_vmusers
				where virtuemart_user_id = " . $_orderData->virtuemart_user_id;
			$db->setQuery($sql);
			$result = $db->loadObjectList();
			
			if( sizeof($result) > 0 ) {
				$_orderData->order_billTaxAmount = $_orderData->order_billTaxAmount * (1-($result[0]->discount_pp*0.01));
				$_orderData->order_billDiscountAmount = ($_orderData->order_total-$_orderData->order_billTaxAmount) - (($_orderData->order_total-$_orderData->order_billTaxAmount) * (1-($result[0]->discount_pp*0.01)));
			}
		}
		//tiago hack
